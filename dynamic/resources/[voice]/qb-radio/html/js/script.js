$(function() {
    ...
    document.onkeyup = function (data) {
        if (data.key == "Escape") { // Escape key
            $.post('https://dyn-radio/escape', JSON.stringify({}));
        } else if (data.key == "Enter") { // Enter key
            $.post('https://dyn-radio/joinRadio', JSON.stringify({
                channel: $("#channel").val()
            }));
        }
    };
...
$(document).on('click', '#disconnect', function(e){
    ...
    $.post('https://dyn-radio/leaveRadio');
});

$(document).on('click', '#volumeUp', function(e){
    e.preventDefault();

    $.post('https://dyn-radio/volumeUp', JSON.stringify({
        channel: $("#channel").val()
    }));
});
...
$(document).on('click', '#volumeDown', function(e){
    ...
    $.post('https://dyn-radio/volumeDown', JSON.stringify({
        channel: $("#channel").val()
    }));
});
...
$(document).on('click', '#decreaseradiochannel', function(e){
    ...
    $.post('https://dyn-radio/decreaseradiochannel', JSON.stringify({
        channel: $("#channel").val()
    }));
});
...
$(document).on('click', '#increaseradiochannel', function(e){
    ...
    $.post('https://dyn-radio/increaseradiochannel', JSON.stringify({
        channel: $("#channel").val()
    }));
});
...
$(document).on('click', '#poweredOff', function(e){
    ...
    $.post('https://dyn-radio/poweredOff', JSON.stringify({
        channel: $("#channel").val()
    }));
});
