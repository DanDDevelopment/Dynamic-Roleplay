...
function DynCore.Functions.CreateClientCallback(name, cb)
    DynCore.ClientCallbacks[name] = cb
end

function DynCore.Functions.TriggerClientCallback(name, cb, ...)
    if not DynCore.ClientCallbacks[name] then return end
    DynCore.ClientCallbacks[name](cb, ...)
end

function DynCore.Functions.TriggerCallback(name, cb, ...)
    DynCore.ServerCallbacks[name] = cb
    TriggerServerEvent('DynCore:Server:TriggerCallback', name, ...)
end

function DynCore.Debug(resource, obj, depth)
    TriggerServerEvent('DynCore:DebugSomething', resource, obj, depth)
end

-- Player

function DynCore.Functions.GetPlayerData(cb)
    if not cb then return DynCore.PlayerData end
    cb(DynCore.PlayerData)
end
...
function DynCore.Functions.HasItem(items, amount)
    return exports['dyn-inventory']:HasItem(items, amount)
end
...
function DynCore.Functions.LookAtEntity(entity, timeout, speed)
...
function DynCore.Functions.PlayAnim(animDict, animName, upperbodyOnly, duration)
...
function DynCore.Functions.IsWearingGloves()
    local ped = PlayerPedId()
    local armIndex = GetPedDrawableVariation(ped, 3)
    local model = GetEntityModel(ped)
    if model == `mp_m_freemode_01` then
        if DynCore.Shared.MaleNoGloves[armIndex] then
            return false
        end
    else
        if DynCore.Shared.FemaleNoGloves[armIndex] then
            return false
        end
    end
    return true
...
function DynCore.Functions.Notify(text, texttype, length, icon)
    local message = {
        action = 'notify',
...
function DynCore.Functions.Progressbar(name, label, duration, useWhileDead, canCancel, disableControls, animation, prop, propTwo, onFinish, onCancel)
    if GetResourceState('progressbar') ~= 'started' then error('progressbar needs to be started in order for DynCore.Functions.Progressbar to work') end
...
function DynCore.Functions.GetVehicles()
    return GetGamePool('CVehicle')
end

function DynCore.Functions.GetObjects()
...
function DynCore.Functions.GetPlayersFromCoords(coords, distance)
...
function DynCore.Functions.GetClosestPlayer(coords)
    local ped = PlayerPedId()
    if coords then
        coords = type(coords) == 'table' and vec3(coords.x, coords.y, coords.z) or coords
    else
    ...
    local closestPlayers = DynCore.Functions.GetPlayersFromCoords(coords)
...
function DynCore.Functions.GetPeds(ignoreList)
...
function DynCore.Functions.GetClosestPed(coords, ignoreList)
    local ped = PlayerPedId()
    if coords then
        coords = type(coords) == 'table' and vec3(coords.x, coords.y, coords.z) or coords
    else
    ...
    ignoreList = ignoreList or {}
    local peds = DynCore.Functions.GetPeds(ignoreList)
    local closestDistance = -1
    local closestPed = -1
    for i = 1, #peds, 1 do
...
function DynCore.Functions.GetClosestVehicle(coords)
    local ped = PlayerPedId()
...
function DynCore.Functions.GetClosestObject(coords)
    local ped = PlayerPedId()
...
function DynCore.Functions.LoadModel(model)
...
function DynCore.Functions.SpawnVehicle(model, cb, coords, isnetworked, teleportInto)
    local ped = PlayerPedId()
    ...
    DynCore.Functions.LoadModel(model)
...
function DynCore.Functions.DeleteVehicle(vehicle)
    SetEntityAsMissionEntity(vehicle, true, true)
    DeleteVehicle(vehicle)
end

function DynCore.Functions.GetPlate(vehicle)
    if vehicle == 0 then return end
    return DynCore.Shared.Trim(GetVehicleNumberPlateText(vehicle))
end

function DynCore.Functions.GetVehicleLabel(vehicle)
    if vehicle == nil or vehicle == 0 then return end
    return GetLabelText(GetDisplayNameFromVehicleModel(GetEntityModel(vehicle)))
end

function DynCore.Functions.GetVehicleProperties(vehicle)
    if DoesEntityExist(vehicle) then
        ...
        return {
            ...
            plate = DynCore.Functions.GetPlate(vehicle),
            plateIndex = GetVehicleNumberPlateTextIndex(vehicle),
            bodyHealth = DynCore.Shared.Round(GetVehicleBodyHealth(vehicle), 0.1),
            engineHealth = DynCore.Shared.Round(GetVehicleEngineHealth(vehicle), 0.1),
            tankHealth = DynCore.Shared.Round(GetVehiclePetrolTankHealth(vehicle), 0.1),
            fuelLevel = DynCore.Shared.Round(GetVehicleFuelLevel(vehicle), 0.1),
            dirtLevel = DynCore.Shared.Round(GetVehicleDirtLevel(vehicle), 0.1),
            oilLevel = DynCore.Shared.Round(GetVehicleOilLevel(vehicle), 0.1),
            color1 = colorPrimary,
...
function DynCore.Functions.SetVehicleProperties(vehicle, props)
    if DoesEntityExist(vehicle) then
...
function DynCore.Functions.DrawText(x, y, width, height, scale, r, g, b, a, text)
...
function DynCore.Functions.DrawText3D(x, y, z, text)
    -- Use local function instead
...
function DynCore.Functions.RequestAnimDict(animDict)
...
function DynCore.Functions.GetClosestBone(entity, list)
...
function DynCore.Functions.GetBoneDistance(entity, boneType, boneIndex)
    local bone
    if boneType == 1 then
...
function DynCore.Functions.AttachProp(ped, model, boneId, x, y, z, xR, yR, zR, vertex)
    local modelHash = type(model) == 'string' and joaat(model) or model
    local bone = GetPedBoneIndex(ped, boneId)
    DynCore.Functions.LoadModel(modelHash)
...
function DynCore.Functions.SpawnClear(coords, radius)
    if coords then
        coords = type(coords) == 'table' and vec3(coords.x, coords.y, coords.z) or coords
    else
...
function DynCore.Functions.LoadAnimSet(animSet)
...
function DynCore.Functions.LoadParticleDictionary(dictionary)
...
function DynCore.Functions.StartParticleAtCoord(dict, ptName, looped, coords, rot, scale, alpha, color, duration)
    if coords then
        coords = type(coords) == 'table' and vec3(coords.x, coords.y, coords.z) or coords
    else
        coords = GetEntityCoords(PlayerPedId())
    end
    DynCore.Functions.LoadParticleDictionary(dict)
...
function DynCore.Functions.StartParticleOnEntity(dict, ptName, looped, entity, bone, offset, rot, scale, alpha, color, evolution, duration)
    DynCore.Functions.LoadParticleDictionary(dict)
    UseParticleFxAssetNextCall(dict)
...
function DynCore.Functions.GetStreetNametAtCoords(coords)
...
function DynCore.Functions.GetZoneAtCoords(coords)
    return GetLabelText(GetNameOfZone(coords))
end

function DynCore.Functions.GetCardinalDirection(entity)
...
function DynCore.Functions.GetCurrentTime()
    local obj = {}
...
function DynCore.Functions.GetGroundZCoord(coords)
...
function DynCore.Functions.GetGroundHash(entity)
    local coords = GetEntityCoords(entity)
