...
local function CraftItem(craftedItem, requiredItems, amountToCraft, xpEarned, xpType)
    DynCore.Functions.TriggerCallback('crafting:getPlayerInventory', function(inventory)
        ...
        for _, reqItem in pairs(requiredItems) do
            ...
            if itemAmount < reqItem.amount then
                ...
                DynCore.Functions.Notify(string.format(Lang:t('notifications.notenoughMaterials')) .. amountToCraft .. 'x ' .. DynCore.Shared.Items[craftedItem].label, 'error')
                break
            end
        end
        if hasAllMaterials then
            if Config.EnableSkillCheck then
                local success = exports['dyn-minigames']:Skillbar('easy', '12345') -- difficulty and words to enter 
                if success then
                    DynCore.Functions.Progressbar('crafting_item', 'Crafting ' .. DynCore.Shared.Items[craftedItem].label, (math.random(2000, 5000) * amountToCraft), false, true, {
                        ...
                        TriggerServerEvent('dyn-crafting:server:receiveItem', craftedItem, requiredItems, amountToCraft, xpEarned, xpType)
                    end)
                else
                    ...
                    TriggerServerEvent('dyn-crafting:server:removeMaterials', randomItem.item, randomAmount)
                    DynCore.Functions.Notify('Crafting failed, some materials have been lost!', 'error')
                end
            else
                DynCore.Functions.Progressbar('crafting_item', 'Crafting ' .. DynCore.Shared.Items[craftedItem].label, (math.random(2000, 5000) * amountToCraft), false, true, {
                    disableMovement = true,
            ...
            DynCore.Functions.Notify(string.format(Lang:t('notifications.notenoughMaterials')), 'error')
        end
    end)
end
...
local function CraftAmount(craftedItem, requiredItems, xpGain, xpType)
    ...
    if dialog and tonumber(dialog.amount) then
        ...
        if amount > 0 then
            ...
            DynCore.Functions.Notify(string.format(Lang:t('notifications.invalidAmount')), 'error')
        end
    else
        DynCore.Functions.Notify(string.format(Lang:t('notifications.invalidInput')), 'error')
    end
end
...
local function OpenCraftingMenu(benchType)
    ...
    DynCore.Functions.TriggerCallback('crafting:getPlayerInventory', function(inventory)
        ...
        for _, recipe in pairs(recipes) do
            if currentXP >= recipe.xpRequired then
                ...
                for _, reqItem in pairs(recipe.requiredItems) do
                    ...
                    local itemLabel = DynCore.Shared.Items[reqItem.item].label
                ...
                local menuItem = {
                    header = DynCore.Shared.Items[recipe.item].label,
                    txt = itemsText,
                    icon = Config.ImageBasePath .. DynCore.Shared.Items[recipe.item].image,
                    params = {
                        isAction = true,
                        event = function()
        ...
        exports['dyn-menu']:openMenu(menuItems)
    end)
end
...
local function PickupBench(benchType)
    ...
    if DoesEntityExist(entity) then
        DeleteEntity(entity)
        TriggerServerEvent('dyn-crafting:server:addCraftingTable', benchType)
        DynCore.Functions.Notify(string.format(Lang:t('notifications.pickupBench')), 'success')
    end
end

-- Events

RegisterNetEvent('dyn-crafting:client:useCraftingTable', function(benchType)
    local playerPed = PlayerPedId()
    ...
    TriggerServerEvent('dyn-crafting:server:removeCraftingTable', benchType)
    exports['dyn-target']:AddTargetEntity(Config[benchType].object, {
        options = {
            {
                icon = 'fas fa-tools',
