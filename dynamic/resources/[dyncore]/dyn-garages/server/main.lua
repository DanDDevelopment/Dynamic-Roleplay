...
local function filterVehiclesByCategory(vehicles, category)
    ...
    for _, vehicle in pairs(vehicles) do
        local vehicleData = DynCore.Shared.Vehicles[vehicle.vehicle]
...
DynCore.Functions.CreateCallback('dyn-garages:server:getHouseGarage', function(_, cb, house)
    local houseInfo = MySQL.single.await('SELECT * FROM houselocations WHERE name = ?', { house })
    cb(houseInfo)
end)

DynCore.Functions.CreateCallback('dyn-garages:server:GetGarageVehicles', function(source, cb, garage, type, category)
...
local function GetVehicleTypeByModel(model)
    local vehicleData = DynCore.Shared.Vehicles[model]
...
DynCore.Functions.CreateCallback('dyn-garages:server:spawnvehicle', function(source, cb, plate, vehicle, coords)
    local vehType = DynCore.Shared.Vehicles[vehicle] and DynCore.Shared.Vehicles[vehicle].type or GetVehicleTypeByModel(vehicle)
...
DynCore.Functions.CreateCallback('dyn-garages:server:IsSpawnOk', function(_, cb, plate, type)
...
DynCore.Functions.CreateCallback('dyn-garages:server:canDeposit', function(source, cb, plate, type, garage, state)
    local Player = DynCore.Functions.GetPlayer(source)
    ...
    if type == 'house' and not exports['dyn-houses']:hasKey(Player.PlayerData.license, Player.PlayerData.citizenid, Config.Garages[garage].houseName) then
        cb(false)
        return
    end
    if state == 1 then
...
RegisterNetEvent('dyn-garages:server:updateVehicleStats', function(plate, fuel, engine, body)
    local src = source
...
RegisterNetEvent('dyn-garages:server:updateVehicleState', function(state, plate)
    local src = source
    local Player = DynCore.Functions.GetPlayer(src)
    if not Player then return end
...
RegisterNetEvent('dyn-garages:server:UpdateOutsideVehicle', function(plate, vehicleNetID)
...
RegisterNetEvent('dyn-garages:server:trackVehicle', function(plate)
    local src = source
    local vehicleData = OutsideVehicles[plate]
    if vehicleData and DoesEntityExist(vehicleData.entity) then
        TriggerClientEvent('dyn-garages:client:trackVehicle', src, GetEntityCoords(vehicleData.entity))
        TriggerClientEvent('DynCore:Notify', src, Lang:t('success.vehicle_tracked'), 'success')
    else
        TriggerClientEvent('DynCore:Notify', src, Lang:t('error.vehicle_not_tracked'), 'error')
    end
end)

RegisterNetEvent('dyn-garages:server:PayDepotPrice', function(data)
    local src = source
    local Player = DynCore.Functions.GetPlayer(src)
    ...
    MySQL.scalar('SELECT depotprice FROM player_vehicles WHERE plate = ?', { data.plate }, function(result)
        if result then
            ...
            if cashBalance >= depotPrice then
                ...
                TriggerClientEvent('dyn-garages:client:takeOutGarage', src, data)
            elseif bankBalance >= depotPrice then
                Player.Functions.RemoveMoney('bank', depotPrice, 'paid-depot')
                TriggerClientEvent('dyn-garages:client:takeOutGarage', src, data)
            else
                TriggerClientEvent('DynCore:Notify', src, Lang:t('error.not_enough'), 'error')
            end
        end
    end)
end)
...
RegisterNetEvent('dyn-garages:server:syncGarage', function(updatedGarages)
    Config.Garages = updatedGarages
end)

--Call from dyn-phone

DynCore.Functions.CreateCallback('dyn-garages:server:GetPlayerVehicles', function(source, cb)
    local Player = DynCore.Functions.GetPlayer(source)
    local Vehicles = {}
    ...
    MySQL.rawExecute('SELECT * FROM player_vehicles WHERE citizenid = ?', { Player.PlayerData.citizenid }, function(result)
        if result[1] then
            for _, v in pairs(result) do
                local VehicleData = DynCore.Shared.Vehicles[v.vehicle]
