...
local function CheckPlayers(vehicle)
    ...
    Wait(1500)
    DynCore.Functions.DeleteVehicle(vehicle)
end

local function OpenGarageMenu(data)
    DynCore.Functions.TriggerCallback('dyn-garages:server:GetGarageVehicles', function(result)
        if result == nil then return DynCore.Functions.Notify(Lang:t('error.no_vehicles'), 'error', 5000) end
        ...
        for _, v in pairs(result) do
            ...
            local vname = nil
            pcall(function()
                vname = DynCore.Shared.Vehicles[v.vehicle].name
            end)
...
local function DepositVehicle(veh, data)
    local plate = DynCore.Functions.GetPlate(veh)
    DynCore.Functions.TriggerCallback('dyn-garages:server:canDeposit', function(canDeposit)
        if canDeposit then
            local bodyDamage = math.ceil(GetVehicleBodyHealth(veh))
            local engineDamage = math.ceil(GetVehicleEngineHealth(veh))
            local totalFuel = exports[Config.FuelResource]:GetFuel(veh)
            TriggerServerEvent('dyn-mechanicjob:server:SaveVehicleProps', DynCore.Functions.GetVehicleProperties(veh))
            TriggerServerEvent('dyn-garages:server:updateVehicleStats', plate, totalFuel, engineDamage, bodyDamage)
            CheckPlayers(veh)
            if plate then TriggerServerEvent('dyn-garages:server:UpdateOutsideVehicle', plate, nil) end
            DynCore.Functions.Notify(Lang:t('success.vehicle_parked'), 'primary', 4500)
        else
            DynCore.Functions.Notify(Lang:t('error.not_owned'), 'error', 3500)
        end
...
local function CreateBlipsZones()
    ...
    comboZone:onPlayerInOut(function(isPointInside, _, zone)
        if isPointInside then
                    ...
                    if IsControlJustReleased(0, 38) then
                        if GetVehiclePedIsUsing(PlayerPedId()) ~= 0 then
                            ...
                            if not IsVehicleAllowed(zone.data.category, currentVehicle) then
                                DynCore.Functions.Notify(Lang:t('error.not_correct_type'), 'error', 3500)
                                return
                            end
            ...
            exports['dyn-core']:DrawText(displayText, 'left')
        else
            listenForKey = false
...
function GetSpawnPoint(garage)
    ...
    if not location then
        DynCore.Functions.Notify(Lang:t('error.vehicle_occupied'), 'error')
    end
    return location
end
...
RegisterNUICallback('takeOutVehicle', function(data, cb)
    TriggerEvent('dyn-garages:client:takeOutGarage', data)
    cb('ok')
end)

RegisterNUICallback('trackVehicle', function(plate, cb)
    TriggerServerEvent('dyn-garages:server:trackVehicle', plate)
    cb('ok')
end)
...
RegisterNUICallback('takeOutDepo', function(data, cb)
    ...
    if depotPrice ~= 0 then
        TriggerServerEvent('dyn-garages:server:PayDepotPrice', data)
    else
        TriggerEvent('dyn-garages:client:takeOutGarage', data)
    end
    cb('ok')
...
RegisterNetEvent('dyn-garages:client:trackVehicle', function(coords)
...
RegisterNetEvent('dyn-garages:client:takeOutGarage', function(data)
    DynCore.Functions.TriggerCallback('dyn-garages:server:IsSpawnOk', function(spawn)
        if spawn then
            local location = GetSpawnPoint(data.garage)
            if not location then return end
            DynCore.Functions.TriggerCallback('dyn-garages:server:spawnvehicle', function(netId, properties, vehPlate)
                ...
                TriggerServerEvent('dyn-garages:server:updateVehicleState', 0, vehPlate)
            ...
            DynCore.Functions.Notify(Lang:t('error.not_depot'), 'error', 5000)
        end
...
local function CreateHouseZone(index, garage, zoneType)
    ...
    houseComboZones:onPlayerInOut(function(isPointInside, _, zone)
        if isPointInside then
            ...
            exports['dyn-core']:DrawText(Lang:t('info.house_garage'), 'left')
        else
...
RegisterNetEvent('dyn-garages:client:setHouseGarage', function(house, hasKey) -- event sent periodically from housing
    ...
    if Config.Garages[formattedHouseName] then
        ...
        DynCore.Functions.TriggerCallback('dyn-garages:server:getHouseGarage', function(garageInfo) -- create garage if not exist
            ...
            TriggerServerEvent('dyn-garages:server:syncGarage', Config.Garages)
        end, house)
    end
end)

RegisterNetEvent('dyn-garages:client:houseGarageConfig', function(houseGarages)
...
RegisterNetEvent('dyn-garages:client:addHouseGarage', function(house, garageInfo) -- event from housing on garage creation
    local formattedHouseName = string.gsub(string.lower(house), ' ', '')
    Config.Garages[formattedHouseName] = {
        houseName = house,
...
RegisterNetEvent('DynCore:Client:OnGangUpdate', function(gang)
    PlayerGang = gang
end)

RegisterNetEvent('DynCore:Client:OnJobUpdate', function(job)
    PlayerJob = job
end)
