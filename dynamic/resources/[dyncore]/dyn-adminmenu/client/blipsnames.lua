...
CreateThread(function()
    while true do
        ...
        if NetCheck1 or NetCheck2 then
            TriggerServerEvent('dyn-admin:server:GetPlayersForBlips')
        end
    end
end)

RegisterNetEvent('dyn-admin:client:toggleBlips', function()
    if not ShowBlips then
        ShowBlips = true
        NetCheck1 = true
        DynCore.Functions.Notify(Lang:t('success.blips_activated'), 'success')
    else
        ShowBlips = false
        DynCore.Functions.Notify(Lang:t('error.blips_deactivated'), 'error')
    end
end)

RegisterNetEvent('dyn-admin:client:toggleNames', function()
    if not ShowNames then
        ShowNames = true
        NetCheck2 = true
        DynCore.Functions.Notify(Lang:t('success.names_activated'), 'success')
    else
        ShowNames = false
        DynCore.Functions.Notify(Lang:t('error.names_deactivated'), 'error')
    end
end)

RegisterNetEvent('dyn-admin:client:Show', function(players)
