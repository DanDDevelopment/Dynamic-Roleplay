...
RegisterNetEvent('dyn-admin:client:spectate', function(targetPed)
...
RegisterNetEvent('dyn-admin:client:SendReport', function(name, src, msg)
    TriggerServerEvent('dyn-admin:server:SendReport', name, src, msg)
end)
...
RegisterNetEvent('dyn-admin:client:SaveCar', function()
    local ped = PlayerPedId()
    local veh = GetVehiclePedIsIn(ped)

    if veh ~= nil and veh ~= 0 then
        local plate = DynCore.Functions.GetPlate(veh)
        local props = DynCore.Functions.GetVehicleProperties(veh)
        local hash = props.model
        local vehname = getVehicleFromVehList(hash)
        if DynCore.Shared.Vehicles[vehname] ~= nil and next(DynCore.Shared.Vehicles[vehname]) ~= nil then
            TriggerServerEvent('dyn-admin:server:SaveCar', props, DynCore.Shared.Vehicles[vehname], GetHashKey(veh), plate)
        else
            DynCore.Functions.Notify(Lang:t('error.no_store_vehicle_garage'), 'error')
        end
    else
        DynCore.Functions.Notify(Lang:t('error.no_vehicle'), 'error')
    end
end)
...
RegisterNetEvent('dyn-admin:client:SetModel', function(skin)
    local ped = PlayerPedId()
...
RegisterNetEvent('dyn-admin:client:SetSpeed', function(speed)
    local ped = PlayerId()
    if speed == 'fast' then
...
RegisterNetEvent('dyn-admin:client:GiveNuiFocus', function(focus, mouse)
...
RegisterNetEvent('dyn-admin:client:maxmodVehicle', function()
