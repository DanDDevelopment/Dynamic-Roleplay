...
function showBlur() {
    $.post('https://dyn-inventory/showBlur');
}
...
$(document).on("contextmenu", ".item-slot", function(e) {
    ...
    if ($(this).data("item") != null) {
        ...
        $.post("https://dyn-inventory/PlayDropSound", JSON.stringify({}));
...
$(document).on("click", ".weapon-attachments-back", function(e) {
    e.preventDefault();
    $("#DynCore-inventory").css({ display: "block" });
    $("#DynCore-inventory").animate({
            left: 0 + "vw",
        },
        200
    );
...
//     localStorage.setItem('dyn-inventory-color', color);
// }

// const savedColor = localStorage.getItem('dyn-inventory-color');
...
function FormatAttachmentInfo(data) {
    $.post(
        "https://dyn-inventory/GetWeaponData",
        JSON.stringify({
            weapon: data.name,
...
function handleAttachmentDrag() {
    ...
    $(".weapon-attachments-remove").droppable({
        ...
        drop: function(event, ui) {
            $.post(
                "https://dyn-inventory/RemoveAttachment",
                JSON.stringify({
...
$(document).on("click", "#weapon-attachments", function(e) {
    ...
    if (!Inventory.IsWeaponBlocked(ClickedItemData.name)) {
        ...
        $("#DynCore-inventory").animate({
            ...
            200,
            function() {
                $("#DynCore-inventory").css({ display: "none" });
            }
        );
        $(".weapon-attachments-container").animate({
                left: 0 + "vw",
...
$(document).on("click", ".CombineItem", function(e) {
    ...
    if (combineslotData.toData.combinable.anim != null) {
        $.post(
            "https://dyn-inventory/combineWithAnim",
            JSON.stringify({
        ...
        $.post(
            "https://dyn-inventory/combineItem",
            JSON.stringify({
...
function optionSwitch(
    ...
    $.post(
        "https://dyn-inventory/SetInventoryData",
        JSON.stringify({
        ...
        if (
            ...
            if (fromData.amount == $toAmount) {
                ...
                if (
                    ...
                    $.post(
                        "https://dyn-inventory/getCombineItem",
...
function InventoryError($elinv, $elslot) {
    ...
    $.post("https://dyn-inventory/PlayDropFail", JSON.stringify({}));
}
...
(() => {
    ...
    Inventory.Close = function() {
        ...
        $(".ply-iteminfo-container").css("opacity", "0.0");
        $("#DynCore-inventory").fadeOut(300);
        $(".combine-option-container").hide();
        $(".item-slot").remove();
        if ($("#rob-money").length) {
            $("#rob-money").remove();
        }
        $.post("https://dyn-inventory/CloseInventory", JSON.stringify({}));

        if (AttachmentScreenActive) {
            $("#DynCore-inventory").css({ left: "0vw" });
