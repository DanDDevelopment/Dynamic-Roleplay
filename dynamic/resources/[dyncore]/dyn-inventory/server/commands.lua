    ...
    if player then
        if itemData then
            ...
            if AddItem(id, itemData['name'], amount, false, info, 'give item command') then
                DynCore.Functions.Notify(source, Lang:t('notify.yhg') .. GetPlayerName(id) .. ' ' .. amount .. ' ' .. itemData['name'] .. '', 'success')
                TriggerClientEvent('dyn-inventory:client:ItemBox', id, itemData, 'add', amount)
                if Player(id).state.inv_busy then TriggerClientEvent('dyn-inventory:client:updateInventory', id) end
            else
                DynCore.Functions.Notify(source, Lang:t('notify.cgitem'), 'error')
            end
        else
            DynCore.Functions.Notify(source, Lang:t('notify.idne'), 'error')
        end
    else
        DynCore.Functions.Notify(source, Lang:t('notify.pdne'), 'error')
    end
end, 'admin')

DynCore.Commands.Add('randomitems', 'Receive random items', {}, false, function(source)
    ...
    for _ = 1, 10, 1 do
        ...
        if emptySlot then
            if AddItem(source, randitem.name, amount, emptySlot, false, 'random items command') then
                TriggerClientEvent('dyn-inventory:client:ItemBox', source, DynCore.Shared.Items[randitem.name], 'add')
                player = DynCore.Functions.GetPlayer(source)
...
DynCore.Commands.Add('clearinv', 'Clear Inventory (Admin Only)', { { name = 'id', help = 'Player ID' } }, false, function(source, args)
    local id = tonumber(args[1])
    if not id then
...
RegisterCommand('hotbar', function(source)
    ...
    TriggerClientEvent('dyn-inventory:client:hotbar', source, hotbarItems)
end, false)
...
RegisterCommand('inventory', function(source)
    ...
    if not QBPlayer then return end
    if not QBPlayer or QBPlayer.PlayerData.metadata['isdead'] or QBPlayer.PlayerData.metadata['inlaststand'] or QBPlayer.PlayerData.metadata['ishandcuffed'] then return end
    DynCore.Functions.TriggerClientCallback('dyn-inventory:client:vehicleCheck', source, function(inventory, class)
