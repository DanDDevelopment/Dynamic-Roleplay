...
function SetInventory(source, items)
    ...
    if not Player.Offline then
        ...
        TriggerEvent('dyn-log:server:CreateLog', 'playerinventory', 'SetInventory', 'blue', logMessage)
    end
end
...
function UseItem(itemName, ...)
    local itemData = DynCore.Functions.CanUseItem(itemName)
...
function GetSlots(identifier)
    ...
    local player = DynCore.Functions.GetPlayer(identifier)
    if player then
...
function CanAddItem(source, item, amount)
    ...
    local itemData = DynCore.Shared.Items[item:lower()]
...
function ClearInventory(source, filterItems)
    ...
    if not player.Offline then
        ...
        TriggerEvent('dyn-log:server:CreateLog', 'playerinventory', 'ClearInventory', 'red', logMessage)
        if Player(source).state.inv_busy then TriggerClientEvent('dyn-inventory:client:updateInventory', source) end
    end
end
...
-- Finally, it triggers the 'dyn-inventory:client:closeInv' event for the given source.
function CloseInventory(source, identifier)
    ...
    TriggerClientEvent('dyn-inventory:client:closeInv', source)
end
...
function OpenInventoryById(source, targetId)
    local QBPlayer = DynCore.Functions.GetPlayer(source)
    local TargetPlayer = DynCore.Functions.GetPlayer(tonumber(targetId))
    ...
    TriggerClientEvent('dyn-inventory:client:openInventory', source, playerItems, formattedInventory)
end
...
function OpenInventory(source, identifier, data)
    ...
    if not identifier then
        ...
        TriggerClientEvent('dyn-inventory:client:openInventory', source, QBPlayer.PlayerData.items)
        return
    end
    ...
    if inventory and inventory.isOpen then
        TriggerClientEvent('DynCore:Notify', source, 'This inventory is currently in use', 'error')
        return
    end
    ...
    TriggerClientEvent('dyn-inventory:client:openInventory', source, QBPlayer.PlayerData.items, formattedInventory)
end
...
function AddItem(identifier, item, amount, slot, info, reason)
    ...
    if not updated then
        ...
        if DynCore.Shared.SplitStr(item, '_')[1] == 'weapon' then
            if not inventory[slot].info.serie then
                inventory[slot].info.serie = tostring(DynCore.Shared.RandomInt(2) .. DynCore.Shared.RandomStr(3) .. DynCore.Shared.RandomInt(1) .. DynCore.Shared.RandomStr(2) .. DynCore.Shared.RandomInt(3) .. DynCore.Shared.RandomStr(4))
            end
    ...
    local resourceName = GetInvokingResource() or 'dyn-inventory'
    TriggerEvent(
        'dyn-log:server:CreateLog',
        'playerinventory',
        'Item Added',
        'green',
...
function RemoveItem(identifier, item, amount, slot, reason)
    if not DynCore.Shared.Items[item:lower()] then
