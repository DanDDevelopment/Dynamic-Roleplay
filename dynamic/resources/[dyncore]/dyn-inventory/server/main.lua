...
local function LoadInventory(source, citizenid)
	...
	for _, item in pairs(inventory) do
		if item then
			local itemInfo = DynCore.Shared.Items[item.name:lower()]
			if itemInfo then
...
local function AddItem(source, item, amount, slot, info, created)
	...
	local itemInfo = DynCore.Shared.Items[item:lower()]
	local time = os.time()
	if not created then
	...
	if not itemInfo and not Player.Offline then
		DynCore.Functions.Notify(source, "Item does not exist", 'error')
		return false
	end
	...
	if itemInfo['type'] == 'weapon' then
		info.serie = info.serie or tostring(DynCore.Shared.RandomInt(2) .. DynCore.Shared.RandomStr(3) .. DynCore.Shared.RandomInt(1) .. DynCore.Shared.RandomStr(2) .. DynCore.Shared.RandomInt(3) .. DynCore.Shared.RandomStr(4))
		info.quality = info.quality or 100
	end
	if (totalWeight + (itemInfo['weight'] * amount)) <= Config.MaxInventoryWeight then
		if (slot and Player.PlayerData.items[slot]) and (Player.PlayerData.items[slot].name:lower() == item:lower()) and (itemInfo['type'] == 'item' and not itemInfo['unique']) then
			...
			TriggerEvent('dyn-log:server:CreateLog', 'playerinventory', 'AddItem', 'green', '**' .. GetPlayerName(source) .. ' (citizenid: ' .. Player.PlayerData.citizenid .. ' | id: ' .. source .. ')** got item: [slot:' .. slot .. '], itemname: ' .. Player.PlayerData.items[slot].name .. ', added amount: ' .. amount .. ', new total amount: ' .. Player.PlayerData.items[slot].amount)
			return true
			else
				for i = 1, Config.MaxInventorySlots, 1 do
					if Player.PlayerData.items[i] == nil then
						...
						Player.Functions.SetPlayerData("items", Player.PlayerData.items)
						if Player.Offline then return true end
						TriggerEvent('dyn-log:server:CreateLog', 'playerinventory', 'AddItem', 'green', '**' .. GetPlayerName(source) .. ' (citizenid: ' .. Player.PlayerData.citizenid .. ' | id: ' .. source .. ')** got item: [slot:' .. i .. '], itemname: ' .. Player.PlayerData.items[i].name .. ', added amount: ' .. amount .. ', new total amount: ' .. Player.PlayerData.items[i].amount)
						return true
					end
				end
			end
	...
	elseif not Player.Offline then
		DynCore.Functions.Notify(source, "Inventory too full", 'error')
	end
	return false
end
	...
	if slot then
		if Player.PlayerData.items[slot].amount > amount then
			...
			if not Player.Offline then
				TriggerEvent('dyn-log:server:CreateLog', 'playerinventory', 'RemoveItem', 'red', '**' .. GetPlayerName(source) .. ' (citizenid: ' .. Player.PlayerData.citizenid .. ' | id: ' .. source .. ')** lost item: [slot:' .. slot .. '], itemname: ' .. Player.PlayerData.items[slot].name .. ', removed amount: ' .. amount .. ', new total amount: ' .. Player.PlayerData.items[slot].amount)
			end
		...
		elseif Player.PlayerData.items[slot].amount == amount then
			...
			TriggerEvent('dyn-log:server:CreateLog', 'playerinventory', 'RemoveItem', 'red', '**' .. GetPlayerName(source) .. ' (citizenid: ' .. Player.PlayerData.citizenid .. ' | id: ' .. source .. ')** lost item: [slot:' .. slot .. '], itemname: ' .. item .. ', removed amount: ' .. amount .. ', item removed')
		...
		for _, _slot in pairs(slots) do
			if Player.PlayerData.items[_slot].amount > amountToRemove then
				...
				if not Player.Offline then
					TriggerEvent('dyn-log:server:CreateLog', 'playerinventory', 'RemoveItem', 'red', '**' .. GetPlayerName(source) .. ' (citizenid: ' .. Player.PlayerData.citizenid .. ' | id: ' .. source .. ')** lost item: [slot:' .. _slot .. '], itemname: ' .. Player.PlayerData.items[_slot].name .. ', removed amount: ' .. amount .. ', new total amount: ' .. Player.PlayerData.items[_slot].amount)
				end
			...
			elseif Player.PlayerData.items[_slot].amount == amountToRemove then
				...
				TriggerEvent('dyn-log:server:CreateLog', 'playerinventory', 'RemoveItem', 'red', '**' .. GetPlayerName(source) .. ' (citizenid: ' .. Player.PlayerData.citizenid .. ' | id: ' .. source .. ')** lost item: [slot:' .. _slot .. '], itemname: ' .. item .. ', removed amount: ' .. amount .. ', item removed')
...
local function ClearInventory(source, filterItems)
	...
	TriggerEvent('dyn-log:server:CreateLog', 'playerinventory', 'ClearInventory', 'red', '**' .. GetPlayerName(source) .. ' (citizenid: ' .. Player.PlayerData.citizenid .. ' | id: ' .. source .. ')** inventory cleared')
end
...
local function SetInventory(source, items)
	...
	TriggerEvent('dyn-log:server:CreateLog', 'playerinventory', 'SetInventory', 'blue', '**' .. GetPlayerName(source) .. ' (citizenid: ' .. Player.PlayerData.citizenid .. ' | id: ' .. source .. ')** items set: ' .. json.encode(items))
end
...
local function CreateUsableItem(itemName, data)
	DynCore.Functions.CreateUseableItem(itemName, data)
end
...
local function GetUsableItem(itemName)
	return DynCore.Functions.CanUseItem(itemName)
end

exports("GetUsableItem", GetUsableItem)

---Use an item from the DynCore.UsableItems table if a callback is present
...
local function AddToStash(stashId, slot, otherslot, itemName, amount, info, created)
	amount = tonumber(amount) or 1
	local ItemData = DynCore.Shared.Items[itemName]
	if not ItemData.unique then
		if Stashes[stashId].items[slot] and Stashes[stashId].items[slot].name == itemName then
			...
			local itemInfo = DynCore.Shared.Items[itemName:lower()]
...
local function CreateNewDrop(source, fromSlot, toSlot, itemAmount, created)
	...
	if RemoveItem(source, itemData.name, itemAmount, itemData.slot) then
		...
		local itemInfo = DynCore.Shared.Items[itemData.name:lower()]
		...
		TriggerEvent("dyn-log:server:CreateLog", "drop", "New Item Drop", "red", "**".. GetPlayerName(source) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..source.."*) dropped new item; name: **"..itemData.name.."**, amount: **" .. itemAmount .. "**")
		...
		TriggerClientEvent("DynCore:Notify", source, "You don't have this item!", "error")
		return
	end
end
...
local function OpenInventory(name, id, other, origin)
	...
	if ply.state.inv_busy then
		return DynCore.Functions.Notify(src, Lang:t("notify.noaccess"), 'error')
	end
	if name and id then
		local secondInv = {}
		if name == "stash" then
			if Stashes[id] then
				if Stashes[id].isOpen then
					local Target = DynCore.Functions.GetPlayer(Stashes[id].isOpen)
					if Target then
		...
		elseif name == "trunk" then
			if Trunks[id] then
				if Trunks[id].isOpen then
					local Target = DynCore.Functions.GetPlayer(Trunks[id].isOpen)
					if Target then
			...
			if (Trunks[id] and Trunks[id].isOpen) or (DynCore.Shared.SplitStr(id, "PLZI")[2] and (Player.PlayerData.job.name ~= "police" or Player.PlayerData.job.type ~= "leo")) then
				secondInv.name = "none-inv"
		...
		elseif name == "glovebox" then
			if Gloveboxes[id] then
				if Gloveboxes[id].isOpen then
					local Target = DynCore.Functions.GetPlayer(Gloveboxes[id].isOpen)
					if Target then
		...
		elseif name == "otherplayer" then
			local OtherPlayer = DynCore.Functions.GetPlayer(tonumber(id))
			if OtherPlayer then
			...
			if Drops[id] then
				if Drops[id].isOpen then
					local Target = DynCore.Functions.GetPlayer(Drops[id].isOpen)
					if Target then
...
AddEventHandler('DynCore:Server:PlayerLoaded', function(Player)
	DynCore.Functions.AddPlayerMethod(Player.PlayerData.source, "AddItem", function(item, amount, slot, info)
		return AddItem(Player.PlayerData.source, item, amount, slot, info)
	end)

	DynCore.Functions.AddPlayerMethod(Player.PlayerData.source, "RemoveItem", function(item, amount, slot)
		return RemoveItem(Player.PlayerData.source, item, amount, slot)
	end)

	DynCore.Functions.AddPlayerMethod(Player.PlayerData.source, "GetItemBySlot", function(slot)
		return GetItemBySlot(Player.PlayerData.source, slot)
	end)

	DynCore.Functions.AddPlayerMethod(Player.PlayerData.source, "GetItemByName", function(item)
		return GetItemByName(Player.PlayerData.source, item)
	end)

	DynCore.Functions.AddPlayerMethod(Player.PlayerData.source, "GetItemsByName", function(item)
		return GetItemsByName(Player.PlayerData.source, item)
	end)

	DynCore.Functions.AddPlayerMethod(Player.PlayerData.source, "ClearInventory", function(filterItems)
		ClearInventory(Player.PlayerData.source, filterItems)
	end)

	DynCore.Functions.AddPlayerMethod(Player.PlayerData.source, "SetInventory", function(items)
...
AddEventHandler('onResourceStart', function(resourceName)
	...
	local Players = DynCore.Functions.GetQBPlayers()
	for k in pairs(Players) do
		DynCore.Functions.AddPlayerMethod(k, "AddItem", function(item, amount, slot, info)
			return AddItem(k, item, amount, slot, info)
		end)

		DynCore.Functions.AddPlayerMethod(k, "RemoveItem", function(item, amount, slot)
			return RemoveItem(k, item, amount, slot)
		end)

		DynCore.Functions.AddPlayerMethod(k, "GetItemBySlot", function(slot)
			return GetItemBySlot(k, slot)
		end)

		DynCore.Functions.AddPlayerMethod(k, "GetItemByName", function(item)
			return GetItemByName(k, item)
		end)

		DynCore.Functions.AddPlayerMethod(k, "GetItemsByName", function(item)
			return GetItemsByName(k, item)
		end)

		DynCore.Functions.AddPlayerMethod(k, "ClearInventory", function(filterItems)
			ClearInventory(k, filterItems)
		end)

		DynCore.Functions.AddPlayerMethod(k, "SetInventory", function(items)
			SetInventory(k, items)
		end)
	end
end)

RegisterNetEvent('DynCore:Server:UpdateObject', function()
...
RegisterNetEvent('inventory:server:combineItem', function(item, fromItem, toItem)
	...
	local recipe = DynCore.Shared.Items[toItem.name].combinable

	if recipe and recipe.reward ~= item then return end
	if not recipeContains(recipe, fromItem) then return end

	TriggerClientEvent('inventory:client:ItemBox', src, DynCore.Shared.Items[item], 'add')
	AddItem(src, item, 1)
...
RegisterNetEvent('inventory:server:UseItemSlot', function(slot)
	...
	if itemData then
	local itemInfo = DynCore.Shared.Items[itemData.name]
	...
	elseif itemData.useable then
		...
		TriggerClientEvent('inventory:client:ItemBox', src, itemInfo, "use")
					...
					if itemInfo['delete'] and RemoveItem(src,itemData.name,1,slot) then
						...
						TriggerClientEvent("DynCore:Notify", src, "You can't use this item", "error")
					end
				end
			else
				UseItem(itemData.name, src, itemData)
...
RegisterNetEvent('inventory:server:SetInventoryData', function(fromInventory, toInventory, fromSlot, toSlot, fromAmount, toAmount)
	...
	if (fromInventory == "player" or fromInventory == "hotbar") and (DynCore.Shared.SplitStr(toInventory, "-")[1] == "itemshop" or toInventory == "crafting") then
		return
	end
	...
	if fromInventory == "player" or fromInventory == "hotbar" then
		...
		if fromItemData and fromItemData.amount >= fromAmount then
			if toInventory == "player" or toInventory == "hotbar" then
                ...
                if toItemData ~= nil then
					...
					if toItemData.name ~= fromItemData.name then
                        ...
                        TriggerEvent("dyn-log:server:CreateLog", "anticheat", "Dupe log", "red", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | *"..src.."*) swapped item; name: **"..toItemData.name.."**, amount: **" .. toAmount .. "** with name: **" .. fromItemData.name .. "**, amount: **" .. fromAmount.. "**")
				end
                end
                AddItem(src, fromItemData.name, fromAmount, toSlot, fromItemData.info, fromItemData["created"])
			elseif DynCore.Shared.SplitStr(toInventory, "-")[1] == "otherplayer" then
				local playerId = tonumber(DynCore.Shared.SplitStr(toInventory, "-")[2])
				...
				TriggerClientEvent("inventory:client:CheckWeapon", src, fromItemData.name)
                if toItemData ~= nil then
					local itemInfo = DynCore.Shared.Items[toItemData.name:lower()]
                    local toAmount = tonumber(toAmount) ~= nil and tonumber(toAmount) or toItemData.amount
					...
					if toItemData.name ~= fromItemData.name then
						...
						TriggerEvent("dyn-log:server:CreateLog", "robbing", "Swapped Item", "orange", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | *"..src.."*) swapped item; name: **"..itemInfo["name"].."**, amount: **" .. toAmount .. "** with name: **" .. fromItemData.name .. "**, amount: **" .. fromAmount.. "** with player: **".. GetPlayerName(OtherPlayer.PlayerData.source) .. "** (citizenid: *"..OtherPlayer.PlayerData.citizenid.."* | id: *"..OtherPlayer.PlayerData.source.."*)")
					end
				else
                        TriggerEvent("dyn-log:server:CreateLog", "anticheat", "Dupe log", "red", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | *"..src.."*) swapped item; name: **"..itemInfo["name"].."**, amount: **" .. toAmount .. "** with name: **" .. fromItemData.name .. "**, amount: **" .. fromAmount.. "** with player: **".. GetPlayerName(OtherPlayer.PlayerData.source) .. "** (citizenid: *"..OtherPlayer.PlayerData.citizenid.."* | id: *"..OtherPlayer.PlayerData.source.."*)")
                    end
                else
					local itemInfo = DynCore.Shared.Items[fromItemData.name:lower()]
					TriggerEvent("dyn-log:server:CreateLog", "robbing", "Dropped Item", "red", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | *"..src.."*) dropped new item; name: **"..itemInfo["name"].."**, amount: **" .. fromAmount .. "** to player: **".. GetPlayerName(OtherPlayer.PlayerData.source) .. "** (citizenid: *"..OtherPlayer.PlayerData.citizenid.."* | id: *"..OtherPlayer.PlayerData.source.."*)")
				end
				local itemInfo = DynCore.Shared.Items[fromItemData.name:lower()]
                AddItem(playerId, itemInfo["name"], fromAmount, toSlot, fromItemData.info, itemInfo["created"])
			elseif DynCore.Shared.SplitStr(toInventory, "-")[1] == "trunk" then
				local plate = DynCore.Shared.SplitStr(toInventory, "-")[2]
                ...
                if toItemData ~= nil then
					...
					if toItemData.name ~= fromItemData.name then
						RemoveFromTrunk(plate, fromSlot, itemInfo["name"], toAmount)
                            Player.Functions.AddItem(toItemData.name, toAmount, fromSlot, toItemData.info)
						TriggerEvent("dyn-log:server:CreateLog", "trunk", "Swapped Item", "orange", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) swapped item; name: **"..itemInfo["name"].."**, amount: **" .. toAmount .. "** with name: **" .. fromItemData.name .. "**, amount: **" .. fromAmount .. "** - plate: *" .. plate .. "*")
					end
				else
                        TriggerEvent("dyn-log:server:CreateLog", "anticheat", "Dupe log", "red", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | *"..src.."*) swapped item; name: **"..itemInfo["name"].."**, amount: **" .. toAmount .. "** with name: **" .. fromItemData.name .. "**, amount: **" .. fromAmount.. "** - plate: *" .. plate .. "*")
                    end
                else
					local itemInfo = DynCore.Shared.Items[fromItemData.name:lower()]
					TriggerEvent("dyn-log:server:CreateLog", "trunk", "Dropped Item", "red", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) dropped new item; name: **"..itemInfo["name"].."**, amount: **" .. fromAmount .. "** - plate: *" .. plate .. "*")
				end
				local itemInfo = DynCore.Shared.Items[fromItemData.name:lower()]
                AddToTrunk(plate, toSlot, fromSlot, itemInfo["name"], fromAmount, fromItemData.info, itemInfo["created"])
			elseif DynCore.Shared.SplitStr(toInventory, "-")[1] == "glovebox" then
				local plate = DynCore.Shared.SplitStr(toInventory, "-")[2]
                ...
                if toItemData ~= nil then
					...
					if toItemData.name ~= fromItemData.name then
						RemoveFromGlovebox(plate, fromSlot, itemInfo["name"], toAmount)
                            Player.Functions.AddItem(toItemData.name, toAmount, fromSlot, toItemData.info)
						TriggerEvent("dyn-log:server:CreateLog", "glovebox", "Swapped Item", "orange", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) swapped item; name: **"..itemInfo["name"].."**, amount: **" .. toAmount .. "** with name: **" .. fromItemData.name .. "**, amount: **" .. fromAmount .. "** - plate: *" .. plate .. "*")
					end
				else
                        TriggerEvent("dyn-log:server:CreateLog", "anticheat", "Dupe log", "red", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | *"..src.."*) swapped item; name: **"..itemInfo["name"].."**, amount: **" .. toAmount .. "** with name: **" .. fromItemData.name .. "**, amount: **" .. fromAmount.. "** - plate: *" .. plate .. "*")
                    end
					...
					local itemInfo = DynCore.Shared.Items[fromItemData.name:lower()]
					TriggerEvent("dyn-log:server:CreateLog", "glovebox", "Dropped Item", "red", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) dropped new item; name: **"..itemInfo["name"].."**, amount: **" .. fromAmount .. "** - plate: *" .. plate .. "*")
				end
				local itemInfo = DynCore.Shared.Items[fromItemData.name:lower()]
                AddToGlovebox(plate, toSlot, fromSlot, itemInfo["name"], fromAmount, fromItemData.info, itemInfo["created"])
			elseif DynCore.Shared.SplitStr(toInventory, "-")[1] == "stash" then
				local stashId = DynCore.Shared.SplitStr(toInventory, "-")[2]
                ...
                if toItemData ~= nil then
					...
					if toItemData.name ~= fromItemData.name then
						...
						RemoveFromStash(stashId, toSlot, itemInfo["name"], toAmount)
                            Player.Functions.AddItem(toItemData.name, toAmount, fromSlot, toItemData.info)
						TriggerEvent("dyn-log:server:CreateLog", "stash", "Swapped Item", "orange", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) swapped item; name: **"..itemInfo["name"].."**, amount: **" .. toAmount .. "** with name: **" .. fromItemData.name .. "**, amount: **" .. fromAmount .. "** - stash: *" .. stashId .. "*")
					end
				else
                        TriggerEvent("dyn-log:server:CreateLog", "anticheat", "Dupe log", "red", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | *"..src.."*) swapped item; name: **"..itemInfo["name"].."**, amount: **" .. toAmount .. "** with name: **" .. fromItemData.name .. "**, amount: **" .. fromAmount.. "** - stash: *" .. stashId .. "*")
                    end
                else
					local itemInfo = DynCore.Shared.Items[fromItemData.name:lower()]
					TriggerEvent("dyn-log:server:CreateLog", "stash", "Dropped Item", "red", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) dropped new item; name: **"..itemInfo["name"].."**, amount: **" .. fromAmount .. "** - stash: *" .. stashId .. "*")
				end
				local itemInfo = DynCore.Shared.Items[fromItemData.name:lower()]
                AddToStash(stashId, toSlot, fromSlot, itemInfo["name"], fromAmount, fromItemData.info, itemInfo["created"])
			elseif DynCore.Shared.SplitStr(toInventory, "-")[1] == "traphouse" then
				-- Traphouse
				local traphouseId = DynCore.Shared.SplitStr(toInventory, "_")[2]
				local toItemData = exports['dyn-traphouse']:GetInventoryData(traphouseId, toSlot)
				local IsItemValid = exports['dyn-traphouse']:CanItemBeSaled(fromItemData.name:lower())
				if IsItemValid then
					RemoveItem(src, fromItemData.name, fromAmount, fromSlot)
					TriggerClientEvent("inventory:client:CheckWeapon", src, fromItemData.name)
                    if toItemData ~= nil then
                        ...
                        if toItemData.amount >= toAmount then
						if toItemData.name ~= fromItemData.name then
							exports['dyn-traphouse']:RemoveHouseItem(traphouseId, fromSlot, itemInfo["name"], toAmount)
                                Player.Functions.AddItem(toItemData.name, toAmount, fromSlot, toItemData.info)
							TriggerEvent("dyn-log:server:CreateLog", "traphouse", "Swapped Item", "orange", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) swapped item; name: **"..itemInfo["name"].."**, amount: **" .. toAmount .. "** with name: **" .. fromItemData.name .. "**, amount: **" .. fromAmount .. "** - traphouse: *" .. traphouseId .. "*")
						end
					else
                            TriggerEvent("dyn-log:server:CreateLog", "anticheat", "Dupe log", "red", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | *"..src.."*) swapped item; name: **"..itemInfo["name"].."**, amount: **" .. toAmount .. "** with name: **" .. fromItemData.name .. "**, amount: **" .. fromAmount.. "** - traphouse: *" .. traphouseId .. "*")
                        end
                    else
						local itemInfo = DynCore.Shared.Items[fromItemData.name:lower()]
						TriggerEvent("dyn-log:server:CreateLog", "traphouse", "Dropped Item", "red", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) dropped new item; name: **"..itemInfo["name"].."**, amount: **" .. fromAmount .. "** - traphouse: *" .. traphouseId .. "*")
					end
					local itemInfo = DynCore.Shared.Items[fromItemData.name:lower()]
					exports['dyn-traphouse']:AddHouseItem(traphouseId, toSlot, itemInfo["name"], fromAmount, fromItemData.info, src)
				else
					TriggerClientEvent('DynCore:Notify', src, "You can\'t sell this item..", 'error')
				end
			else
				-- drop
				...
				if toInventory == nil or toInventory == 0 then
                    ...
                    if toItemData ~= nil then
						...
						if toItemData.name ~= fromItemData.name then
							...
							TriggerEvent("dyn-log:server:CreateLog", "drop", "Swapped Item", "orange", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) swapped item; name: **"..itemInfo["name"].."**, amount: **" .. toAmount .. "** with name: **" .. fromItemData.name .. "**, amount: **" .. fromAmount .. "** - dropid: *" .. toInventory .. "*")
						end
					else
                            TriggerEvent("dyn-log:server:CreateLog", "anticheat", "Dupe log", "red", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | *"..src.."*) swapped item; name: **"..itemInfo["name"].."**, amount: **" .. toAmount .. "** with name: **" .. fromItemData.name .. "**, amount: **" .. fromAmount.. "** - dropid: *" .. toInventory .. "*")
                        end
                    else
						local itemInfo = DynCore.Shared.Items[fromItemData.name:lower()]
						TriggerEvent("dyn-log:server:CreateLog", "drop", "Dropped Item", "red", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) dropped new item; name: **"..itemInfo["name"].."**, amount: **" .. fromAmount .. "** - dropid: *" .. toInventory .. "*")
					end
					local itemInfo = DynCore.Shared.Items[fromItemData.name:lower()]
			...
			TriggerClientEvent("DynCore:Notify", src, "You don\'t have this item!", "error")
		end
	elseif DynCore.Shared.SplitStr(fromInventory, "-")[1] == "otherplayer" then
		local playerId = tonumber(DynCore.Shared.SplitStr(fromInventory, "-")[2])
		local OtherPlayer = DynCore.Functions.GetPlayer(playerId)
		...
		if fromItemData and fromItemData.amount >= fromAmount then
			...
			if toInventory == "player" or toInventory == "hotbar" then
                ...
                if toItemData ~= nil then
					itemInfo = DynCore.Shared.Items[toItemData.name:lower()]
                    local toAmount = tonumber(toAmount) ~= nil and tonumber(toAmount) or toItemData.amount
                    if toItemData.amount >= toAmount then
					if toItemData.name ~= fromItemData.name then
				...
				itemInfo = DynCore.Shared.Items[fromItemData.name:lower()]
                AddItem(playerId, itemInfo["name"], fromAmount, toSlot, fromItemData.info, itemInfo["created"])
			end
		else
			DynCore.Functions.Notify(src, "Item doesn't exist", "error")
		end
	elseif DynCore.Shared.SplitStr(fromInventory, "-")[1] == "trunk" then
		local plate = DynCore.Shared.SplitStr(fromInventory, "-")[2]
		...
		if fromItemData and fromItemData.amount >= fromAmount then
			...
			if toInventory == "player" or toInventory == "hotbar" then
                ...
                if toItemData ~= nil then
					...
					if toItemData.name ~= fromItemData.name then
						...
						TriggerEvent("dyn-log:server:CreateLog", "trunk", "Swapped Item", "orange", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) swapped item; name: **"..toItemData.name.."**, amount: **" .. toAmount .. "** with item; name: **"..itemInfo["name"].."**, amount: **" .. toAmount .. "** plate: *" .. plate .. "*")
					else
						TriggerEvent("dyn-log:server:CreateLog", "trunk", "Stacked Item", "orange", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) stacked item; name: **"..toItemData.name.."**, amount: **" .. toAmount .. "** from plate: *" .. plate .. "*")
					end
				else
                        TriggerEvent("dyn-log:server:CreateLog", "anticheat", "Dupe log", "red", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | *"..src.."*) swapped item; name: **"..toItemData.name.."**, amount: **" .. toAmount .. "** with name: **" .. itemInfo["name"] .. "**, amount: **" .. toAmount.. "** plate: *" .. plate .. "*")
                    end
                else
					TriggerEvent("dyn-log:server:CreateLog", "trunk", "Received Item", "green", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) received item; name: **"..fromItemData.name.."**, amount: **" .. fromAmount.. "** plate: *" .. plate .. "*")
				end
                AddItem(src, fromItemData.name, fromAmount, toSlot, fromItemData.info, fromItemData["created"])
			else
				local toItemData = Trunks[plate].items[toSlot]
            ...
            DynCore.Functions.Notify(src, Lang:t("notify.itemexist"), "error")
		end
	elseif DynCore.Shared.SplitStr(fromInventory, "-")[1] == "glovebox" then
		local plate = DynCore.Shared.SplitStr(fromInventory, "-")[2]
			...
			if toInventory == "player" or toInventory == "hotbar" then
                ...
                if toItemData ~= nil then
					...
					if toItemData.name ~= fromItemData.name then
						...
						TriggerEvent("dyn-log:server:CreateLog", "glovebox", "Swapped", "orange", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src..")* swapped item; name: **"..toItemData.name.."**, amount: **" .. toAmount .. "** with item; name: **"..itemInfo["name"].."**, amount: **" .. toAmount .. "** plate: *" .. plate .. "*")
					else
						TriggerEvent("dyn-log:server:CreateLog", "glovebox", "Stacked Item", "orange", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) stacked item; name: **"..toItemData.name.."**, amount: **" .. toAmount .. "** from plate: *" .. plate .. "*")
					end
				else
                        TriggerEvent("dyn-log:server:CreateLog", "anticheat", "Dupe log", "red", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | *"..src.."*) swapped item; name: **"..toItemData.name.."**, amount: **" .. toAmount .. "** with name: **" .. itemInfo["name"] .. "**, amount: **" .. toAmount.. "** plate: *" .. plate .. "*")
                    end
                else
					TriggerEvent("dyn-log:server:CreateLog", "glovebox", "Received Item", "green", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) received item; name: **"..fromItemData.name.."**, amount: **" .. fromAmount.. "** plate: *" .. plate .. "*")
				end
                AddItem(src, fromItemData.name, fromAmount, toSlot, fromItemData.info, fromItemData["created"])
			else
				local toItemData = Gloveboxes[plate].items[toSlot]
	...
	elseif DynCore.Shared.SplitStr(fromInventory, "-")[1] == "stash" then
		local stashId = DynCore.Shared.SplitStr(fromInventory, "-")[2]
			...
			if toInventory == "player" or toInventory == "hotbar" then
                ...
                if toItemData ~= nil then
					...
					if toItemData.name ~= fromItemData.name then
						...
						TriggerEvent("dyn-log:server:CreateLog", "stash", "Swapped Item", "orange", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) swapped item; name: **"..toItemData.name.."**, amount: **" .. toAmount .. "** with item; name: **"..fromItemData.name.."**, amount: **" .. fromAmount .. "** stash: *" .. stashId .. "*")
					else
						TriggerEvent("dyn-log:server:CreateLog", "stash", "Stacked Item", "orange", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) stacked item; name: **"..toItemData.name.."**, amount: **" .. toAmount .. "** from stash: *" .. stashId .. "*")
					end
				else
                        TriggerEvent("dyn-log:server:CreateLog", "anticheat", "Dupe log", "red", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) swapped item; name: **"..toItemData.name.."**, amount: **" .. toAmount .. "** with item; name: **"..fromItemData.name.."**, amount: **" .. fromAmount .. "** stash: *" .. stashId .. "*")
                    end
                else
					TriggerEvent("dyn-log:server:CreateLog", "stash", "Received Item", "green", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) received item; name: **"..fromItemData.name.."**, amount: **" .. fromAmount.. "** stash: *" .. stashId .. "*")
				end
	...
	elseif DynCore.Shared.SplitStr(fromInventory, "-")[1] == "traphouse" then
		local traphouseId = DynCore.Shared.SplitStr(fromInventory, "_")[2]
		local fromItemData = exports['dyn-traphouse']:GetInventoryData(traphouseId, fromSlot)
		fromAmount = tonumber(fromAmount) or fromItemData.amount
		if fromItemData and fromItemData.amount >= fromAmount then
			local itemInfo = DynCore.Shared.Items[fromItemData.name:lower()]
			if toInventory == "player" or toInventory == "hotbar" then
				local toItemData = GetItemBySlot(src, toSlot)
				exports['dyn-traphouse']:RemoveHouseItem(traphouseId, fromSlot, itemInfo["name"], fromAmount)
                if toItemData ~= nil then
					itemInfo = DynCore.Shared.Items[toItemData.name:lower()]
                    local toAmount = tonumber(toAmount) ~= nil and tonumber(toAmount) or toItemData.amount
                    if toItemData.amount >= toAmount then
					if toItemData.name ~= fromItemData.name then
                            Player.Functions.RemoveItem(toItemData.name, toAmount, toSlot)
						exports['dyn-traphouse']:AddHouseItem(traphouseId, fromSlot, itemInfo["name"], toAmount, toItemData.info, src)
						TriggerEvent("dyn-log:server:CreateLog", "stash", "Swapped Item", "orange", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) swapped item; name: **"..toItemData.name.."**, amount: **" .. toAmount .. "** with item; name: **"..fromItemData.name.."**, amount: **" .. fromAmount .. "** stash: *" .. traphouseId .. "*")
					else
						TriggerEvent("dyn-log:server:CreateLog", "stash", "Stacked Item", "orange", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) stacked item; name: **"..toItemData.name.."**, amount: **" .. toAmount .. "** from stash: *" .. traphouseId .. "*")
					end
				else
                        TriggerEvent("dyn-log:server:CreateLog", "anticheat", "Dupe log", "red", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) swapped item; name: **"..toItemData.name.."**, amount: **" .. toAmount .. "** with item; name: **"..fromItemData.name.."**, amount: **" .. fromAmount .. "** stash: *" .. traphouseId .. "*")
                    end
                else
					TriggerEvent("dyn-log:server:CreateLog", "stash", "Received Item", "green", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) received item; name: **"..fromItemData.name.."**, amount: **" .. fromAmount.. "** stash: *" .. traphouseId .. "*")
				end
                AddItem(src, fromItemData.name, fromAmount, toSlot, fromItemData.info, fromItemData["created"])
			else
				local toItemData = exports['dyn-traphouse']:GetInventoryData(traphouseId, toSlot)
                ...
                if toItemData ~= nil then
					...
					if toItemData.name ~= fromItemData.name then
                            local itemInfo = DynCore.Shared.Items[toItemData.name:lower()]
						exports['dyn-traphouse']:RemoveHouseItem(traphouseId, toSlot, itemInfo["name"], toAmount)
						exports['dyn-traphouse']:AddHouseItem(traphouseId, fromSlot, itemInfo["name"], toAmount, toItemData.info, src)
					end
                    else
                        TriggerEvent("dyn-log:server:CreateLog", "anticheat", "Dupe log", "red", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) swapped item; name: **"..toItemData.name.."**, amount: **" .. toAmount .. "** with item; name: **"..fromItemData.name.."**, amount: **" .. fromAmount .. "** stash: *" .. traphouseId .. "*")
	...
	elseif DynCore.Shared.SplitStr(fromInventory, "-")[1] == "itemshop" then
		local shopType = DynCore.Shared.SplitStr(fromInventory, "-")[2]
		local itemData = ShopItems[shopType].items[fromSlot]
		local itemInfo = DynCore.Shared.Items[itemData.name:lower()]
		local bankBalance = Player.PlayerData.money["bank"]
		local price = tonumber((itemData.price*fromAmount))

		if DynCore.Shared.SplitStr(shopType, "_")[1] == "Dealer" then
			if DynCore.Shared.SplitStr(itemData.name, "_")[1] == "weapon" then
				price = tonumber(itemData.price)
				if Player.Functions.RemoveMoney("cash", price, "dealer-item-bought") then
					itemData.info.serie = tostring(DynCore.Shared.RandomInt(2) .. DynCore.Shared.RandomStr(3) .. DynCore.Shared.RandomInt(1) .. DynCore.Shared.RandomStr(2) .. DynCore.Shared.RandomInt(3) .. DynCore.Shared.RandomStr(4))
					itemData.info.quality = 100
					AddItem(src, itemData.name, 1, toSlot, itemData.info)
					TriggerClientEvent('dyn-drugs:client:updateDealerItems', src, itemData, 1)
					DynCore.Functions.Notify(src, itemInfo["label"] .. " bought!", "success")
					TriggerEvent("dyn-log:server:CreateLog", "dealers", "Dealer item bought", "green", "**"..GetPlayerName(src) .. "** bought a " .. itemInfo["label"] .. " for $"..price)
				else
					DynCore.Functions.Notify(src, "You don\'t have enough cash..", "error")
				end
			else
				if Player.Functions.RemoveMoney("cash", price, "dealer-item-bought") then
					AddItem(src, itemData.name, fromAmount, toSlot, itemData.info)
					TriggerClientEvent('dyn-drugs:client:updateDealerItems', src, itemData, fromAmount)
					DynCore.Functions.Notify(src, itemInfo["label"] .. " bought!", "success")
					TriggerEvent("dyn-log:server:CreateLog", "dealers", "Dealer item bought", "green", "**"..GetPlayerName(src) .. "** bought a " .. itemInfo["label"] .. "  for $"..price)
				else
					DynCore.Functions.Notify(src, "You don't have enough cash..", "error")
		...
		elseif DynCore.Shared.SplitStr(shopType, "_")[1] == "Itemshop" then
            if Player.Functions.RemoveMoney("cash", price, "itemshop-bought-item") then
                ...
                local imageurl = ("https://cfx-nui-dyn-inventory/html/images/%s.png"):format(itemData.name)
                local notes = "Purchased at Ammunation"
                local owner = Player.PlayerData.charinfo.firstname .. " " .. Player.PlayerData.charinfo.lastname
                local weapClass = 1
                local weapModel = DynCore.Shared.Items[itemData.name].label
                AddItem(src, itemData.name, fromAmount, toSlot, itemData.info)
                TriggerClientEvent('dyn-shops:client:UpdateShop', src, DynCore.Shared.SplitStr(shopType, "_")[2], itemData, fromAmount)
                DynCore.Functions.Notify(src, itemInfo["label"] .. " bought!", "success")
                exports['dyn-mdt']:CreateWeaponInfo(serial, imageurl, notes, owner, weapClass, weapModel)
                TriggerEvent("dyn-log:server:CreateLog", "shops", "Shop item bought", "green", "**"..GetPlayerName(src) .. "** bought a " .. itemInfo["label"] .. " for $"..price)
        elseif bankBalance >= price then
			...
			elseif bankBalance >= price then
				...
				TriggerClientEvent('DynCore:Notify', src, "You don\'t have enough cash..", "error")
			end
		end
	elseif fromInventory == "crafting" then
		...
		if hasCraftItems(src, itemData.costs, fromAmount) then
			...
			TriggerClientEvent('DynCore:Notify', src, "You don't have the right items..", "error")
		end
			...
			if toInventory == "player" or toInventory == "hotbar" then
                ...
                if toItemData ~= nil then
					...
					if toItemData.name ~= fromItemData.name then
						...
						TriggerEvent("dyn-log:server:CreateLog", "drop", "Swapped Item", "orange", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) swapped item; name: **"..toItemData.name.."**, amount: **" .. toAmount .. "** with item; name: **"..fromItemData.name.."**, amount: **" .. fromAmount .. "** - dropid: *" .. fromInventory .. "*")
					else
						TriggerEvent("dyn-log:server:CreateLog", "drop", "Stacked Item", "orange", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) stacked item; name: **"..toItemData.name.."**, amount: **" .. toAmount .. "** - from dropid: *" .. fromInventory .. "*")
					end
				else
                        TriggerEvent("dyn-log:server:CreateLog", "anticheat", "Dupe log", "red", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) swapped item; name: **"..toItemData.name.."**, amount: **" .. toAmount .. "** with item; name: **"..fromItemData.name.."**, amount: **" .. fromAmount .. "** - dropid: *" .. fromInventory .. "*")
                    end
                else
					TriggerEvent("dyn-log:server:CreateLog", "drop", "Received Item", "green", "**".. GetPlayerName(src) .. "** (citizenid: *"..Player.PlayerData.citizenid.."* | id: *"..src.."*) received item; name: **"..fromItemData.name.."**, amount: **" .. fromAmount.. "** -  dropid: *" .. fromInventory .. "*")
				end
                AddItem(src, fromItemData.name, fromAmount, toSlot, fromItemData.info, fromItemData["created"])
			else
				toInventory = tonumber(toInventory)
...
RegisterNetEvent('dyn-inventory:server:SaveStashItems', function(stashId, items)
...
RegisterServerEvent("inventory:server:GiveItem", function(target, name, amount, slot)
    ...
    local OtherPlayer = DynCore.Functions.GetPlayer(target)
    local dist = #(GetEntityCoords(GetPlayerPed(src))-GetEntityCoords(GetPlayerPed(target)))
	if Player == OtherPlayer then return DynCore.Functions.Notify(src, "You can\'t give yourself an item?") end
	if dist > 2 then return DynCore.Functions.Notify(src, "You are too far away to give items!") end
	local item = GetItemBySlot(src, slot)
	if not item then DynCore.Functions.Notify(src, "Item you tried giving not found!"); return end
	if item.name ~= name then DynCore.Functions.Notify(src, "Incorrect item found try again!"); return end
	...
	if amount <= item.amount then
		...
		if RemoveItem(src, item.name, amount, item.slot) then
			if AddItem(target, item.name, amount, false, item.info, item.created) then
				TriggerClientEvent('inventory:client:ItemBox',target, DynCore.Shared.Items[item.name], "add", amount)
				DynCore.Functions.Notify(target, "You Received "..amount..' '..item.label.." From "..Player.PlayerData.charinfo.firstname.." "..Player.PlayerData.charinfo.lastname)
				TriggerClientEvent("inventory:client:UpdatePlayerInventory", target, true)
				TriggerClientEvent('inventory:client:ItemBox',src, DynCore.Shared.Items[item.name], "remove", amount)
				DynCore.Functions.Notify(src, "You gave " .. OtherPlayer.PlayerData.charinfo.firstname.." "..OtherPlayer.PlayerData.charinfo.lastname.. " " .. amount .. " " .. item.label .."!")
				TriggerClientEvent("inventory:client:UpdatePlayerInventory", src, true)
				TriggerClientEvent('dyn-inventory:client:giveAnim', src)
				TriggerClientEvent('dyn-inventory:client:giveAnim', target)
			else
				AddItem(src, item.name, amount, item.slot, item.info, item.created)
				DynCore.Functions.Notify(src, "The other players inventory is full!", "error")
				DynCore.Functions.Notify(target, "The other players inventory is full!", "error")
				TriggerClientEvent("inventory:client:UpdatePlayerInventory", src, false)
				TriggerClientEvent("inventory:client:UpdatePlayerInventory", target, false)
			end
		else
			TriggerClientEvent('DynCore:Notify', src,  "You do not have enough of the item", "error")
		end
	else
		TriggerClientEvent('DynCore:Notify', src, "You do not have enough items to transfer")
	end
end)
...
DynCore.Functions.CreateCallback('dyn-inventory:server:GetStashItems', function(source, cb, stashId)
	cb(GetStashItems(stashId))
end)

DynCore.Functions.CreateCallback('inventory:server:GetCurrentDrops', function(_, cb)
	cb(Drops)
end)

DynCore.Functions.CreateCallback('DynCore:HasItem', function(source, cb, items, amount)
	print("^3DynCore:HasItem is deprecated, please use DynCore.Functions.HasItem, it can be used on both server- and client-side and uses the same arguments.^0")
    local retval = false
    local Player = DynCore.Functions.GetPlayer(source)
...
DynCore.Commands.Add("resetinv", "Reset Inventory (Admin Only)", {{name="type", help="stash/trunk/glovebox"},{name="id/plate", help="ID of stash or license plate"}}, true, function(source, args)
	...
	if invType and invId then
		...
		elseif invType == "stash" then
			...
			TriggerClientEvent('DynCore:Notify', source,  "Not a valid type..", "error")
		end
	else
		TriggerClientEvent('DynCore:Notify', source,  "Arguments not filled out correctly..", "error")
	end
end, "admin")

DynCore.Commands.Add("rob", "Rob Player", {}, false, function(source, args)
	TriggerClientEvent("police:client:RobPlayer", source)
end)

DynCore.Commands.Add("giveitem", "Give An Item (Admin Only)", {{name="id", help="Player ID"},{name="item", help="Name of the item (not a label)"}, {name="amount", help="Amount of items"}}, false, function(source, args)
	local id = tonumber(args[1])
	local Player = DynCore.Functions.GetPlayer(id)
	local amount = tonumber(args[3]) or 1
	local itemData = DynCore.Shared.Items[tostring(args[2]):lower()]
	if Player then
			if itemData then
				-- check iteminfo
				local info = {}
				...
				elseif itemData["name"] == "labkey" then
					info.lab = exports["dyn-methlab"]:GenerateRandomLab()
				elseif itemData["name"] == "printerdocument" then
					info.url = "https://cdn.discordapp.com/attachments/870094209783308299/870104331142189126/Logo_-_Display_Picture_-_Stylized_-_Red.png"
				elseif DynCore.Shared.Items[itemData["name"]]["decay"] and DynCore.Shared.Items[itemData["name"]]["decay"] > 0 then
					info.quality = 100
				end

				if AddItem(id, itemData["name"], amount, false, info) then
					DynCore.Functions.Notify(source, "You Have Given " ..GetPlayerName(id).." "..amount.." "..itemData["name"].. "", "success")
				else
					DynCore.Functions.Notify(source, "Can\'t give item!", "error")
				end
			else
				DynCore.Functions.Notify(source, "Item Does Not Exist", "error")
			end
	else
		DynCore.Functions.Notify(source,  "Player Is Not Online", "error")
	end
end, "admin")

DynCore.Commands.Add("randomitems", "Give Random Items (God Only)", {}, false, function(source, _)
	local filteredItems = {}
	for k, v in pairs(DynCore.Shared.Items) do
		if DynCore.Shared.Items[k]["type"] ~= "weapon" then
	...
	for _ = 1, 10, 1 do
		...
		if AddItem(source, randitem["name"], amount) then
			TriggerClientEvent('inventory:client:ItemBox', source, DynCore.Shared.Items[randitem["name"]], 'add')
            Wait(500)
		end
	end
end, "god")

DynCore.Commands.Add('clearinv', 'Clear Players Inventory (Admin Only)', { { name = 'id', help = 'Player ID' } }, false, function(source, args)
    local playerId = args[1] ~= '' and tonumber(args[1]) or source
    local Player = DynCore.Functions.GetPlayer(playerId)
    if Player then
        ClearInventory(playerId)
    else
        DynCore.Functions.Notify(source, "Player not online", 'error')
    end
end, 'admin')

-- item

-- DynCore.Functions.CreateUseableItem("snowball", function(source, item)
-- 	local Player = DynCore.Functions.GetPlayer(source)
...
CreateUsableItem("driver_license", function(source, item)
	...
	local players = DynCore.Functions.GetPlayers()
...
function ConvertQuality(item)
    ...
    local DecayRate = DynCore.Shared.Items[item.name:lower()]["decay"] ~= nil and DynCore.Shared.Items[item.name:lower()]["decay"] or 0.0
...
DynCore.Functions.CreateCallback('inventory:server:ConvertQuality', function(source, cb, inventory, other)
    local src = source
    local data = {}
    local Player = DynCore.Functions.GetPlayer(src)
    for _, item in pairs(inventory) do
        if item.created then
            if DynCore.Shared.Items[item.name:lower()]["decay"] ~= nil or DynCore.Shared.Items[item.name:lower()]["decay"] ~= 0 then
                if item.info then
    ...
    if other then
		local inventoryType = DynCore.Shared.SplitStr(other.name, "-")[1]
		local uniqueId = DynCore.Shared.SplitStr(other.name, "-")[2]
...
RegisterNetEvent('inventory:server:addTrunkItems', function()
	print('inventory:server:addTrunkItems has been deprecated please use exports[\'dyn-inventory\']:addTrunkItems(plate, items)')
end)
RegisterNetEvent('inventory:server:addGloveboxItems', function()
	print('inventory:server:addGloveboxItems has been deprecated please use exports[\'dyn-inventory\']:addGloveboxItems(plate, items)')
end)
