...
AddEventHandler('DynCore:Server:PlayerLoaded', function(Player)
    DynCore.Functions.AddPlayerMethod(Player.PlayerData.source, 'AddItem', function(item, amount, slot, info, reason)
        return AddItem(Player.PlayerData.source, item, amount, slot, info, reason)
    end)

    DynCore.Functions.AddPlayerMethod(Player.PlayerData.source, 'RemoveItem', function(item, amount, slot, reason)
...
local function checkWeapon(source, item)
    ...
    local weaponInfo = DynCore.Shared.Weapons[weapon]
...
RegisterNetEvent('dyn-inventory:server:openVending', function()
    local src = source
...
RegisterNetEvent('dyn-inventory:server:closeInventory', function(inventory)
    local src = source
    local QBPlayer = DynCore.Functions.GetPlayer(src)
    ...
    if Drops[inventory] then
        ...
        if #Drops[inventory].items == 0 and not Drops[inventory].isOpen then -- if no listeed items in the drop on close
            TriggerClientEvent('dyn-inventory:client:removeDropTarget', -1, Drops[inventory].entityId)
            Wait(500)
...
RegisterNetEvent('dyn-inventory:server:useItem', function(item)
    local itemData = GetItemBySlot(source, item.slot)
    if not itemData then return end
    local itemInfo = DynCore.Shared.Items[itemData.name]
    if itemData.type == 'weapon' then
        TriggerClientEvent('dyn-weapons:client:UseWeapon', source, itemData, itemData.info.quality and itemData.info.quality > 0)
        TriggerClientEvent('dyn-inventory:client:ItemBox', source, itemInfo, 'use')
    else
...
RegisterNetEvent('dyn-inventory:server:openDrop', function(dropId)
    local src = source
    local Player = DynCore.Functions.GetPlayer(src)
    if not Player then return end
    ...
    drop.isOpen = true
    TriggerClientEvent('dyn-inventory:client:openInventory', source, Player.PlayerData.items, formattedInventory)
end)

RegisterNetEvent('dyn-inventory:server:updateDrop', function(dropId, coords)
    Drops[dropId].coords = coords
end)

RegisterNetEvent('dyn-inventory:server:snowball', function(action)
    if action == 'add' then
        AddItem(source, 'weapon_snowball', 1, false, false, 'dyn-inventory:server:snowball')
    elseif action == 'remove' then
        RemoveItem(source, 'weapon_snowball', 1, false, 'dyn-inventory:server:snowball')
    end
end)

-- Callbacks

DynCore.Functions.CreateCallback('dyn-inventory:server:GetCurrentDrops', function(_, cb)
    cb(Drops)
end)

DynCore.Functions.CreateCallback('dyn-inventory:server:createDrop', function(source, cb, item)
    local src = source
    local Player = DynCore.Functions.GetPlayer(src)
    if not Player then
        cb(false)
    ...
    if RemoveItem(src, item.name, item.amount, item.fromSlot, 'dropped item') then
        ...
        if not Drops[newDropId] then
            ...
            TriggerClientEvent('dyn-inventory:client:setupDropTarget', -1, dropId)
        else
...
DynCore.Functions.CreateCallback('dyn-inventory:server:attemptPurchase', function(source, cb, data)
    ...
    if not CanAddItem(source, itemInfo.name, amount) then
        TriggerClientEvent('DynCore:Notify', source, 'Cannot hold item', 'error')
        cb(false)
        return
    end
    ...
    if Player.PlayerData.money.cash >= price then
        ...
        TriggerEvent('dyn-shops:server:UpdateShopItems', shop, itemInfo, amount)
        cb(true)
    else
        TriggerClientEvent('DynCore:Notify', source, 'You do not have enough money', 'error')
        cb(false)
    end
end)

DynCore.Functions.CreateCallback('dyn-inventory:server:giveItem', function(source, cb, target, item, amount)
    local player = DynCore.Functions.GetPlayer(source)
    ...
    local Target = DynCore.Functions.GetPlayer(target)
    ...
    TriggerClientEvent('dyn-inventory:client:giveAnim', source)
    TriggerClientEvent('dyn-inventory:client:ItemBox', source, itemInfo, 'remove', giveAmount)
    TriggerClientEvent('dyn-inventory:client:giveAnim', target)
    TriggerClientEvent('dyn-inventory:client:ItemBox', target, itemInfo, 'add', giveAmount)
    if Player(target).state.inv_busy then TriggerClientEvent('dyn-inventory:client:updateInventory', target) end
    cb(true)
end)
...
local function getItem(inventoryId, src, slot)
    ...
    elseif inventoryId:find('otherplayer-') then
        ...
        local targetPlayer = DynCore.Functions.GetPlayer(targetId)
...
RegisterNetEvent('dyn-inventory:server:SetInventoryData', function(fromInventory, toInventory, fromSlot, toSlot, fromAmount, toAmount)
