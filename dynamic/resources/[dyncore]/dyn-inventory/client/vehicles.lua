...
function CloseTrunk()
    local vehicle, distance = DynCore.Functions.GetClosestVehicle()
...
DynCore.Functions.CreateClientCallback('dyn-inventory:client:vehicleCheck', function(cb)
    ...
    if vehicle ~= 0 and distance < 5 then
        ...
        if #(pos - trunkpos) < 1.5 then
            if GetVehicleDoorLockStatus(vehicle) < 2 then
                ...
                DynCore.Functions.Notify(Lang:t('notify.vlocked'), 'error')
                return
            end
        end
    end
