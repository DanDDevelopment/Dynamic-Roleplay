...
local function FormatWeaponAttachments(itemdata)
    ...
    local WeaponAttachments = exports['dyn-weapons']:getConfigWeaponAttachments()
        ...
        if componentHash then
            for _, attachmentData in pairs(itemdata.info.attachments) do
                if attachmentData.component == componentHash then
                    local label = DynCore.Shared.Items[attachmentType] and DynCore.Shared.Items[attachmentType].label or 'Unknown'
...
RegisterNetEvent('dyn-inventory:client:requiredItems', function(items, bool)
    local itemTable = {}
    if bool then
...
RegisterNetEvent('dyn-inventory:client:hotbar', function(items)
...
RegisterNetEvent('dyn-inventory:client:updateInventory', function()
    SendNUIMessage({
        action = 'update',
...
RegisterNetEvent('dyn-inventory:client:ItemBox', function(itemData, type, amount)
    SendNUIMessage({
        action = 'itemBox',
        item = itemData,
        type = type,
...
RegisterNetEvent('dyn-inventory:server:RobPlayer', function(TargetId)
    SendNUIMessage({
        action = 'RobMoney',
        TargetId = TargetId,
    })
...
RegisterNetEvent('dyn-inventory:client:openInventory', function(items, other)
...
RegisterNUICallback('AttemptPurchase', function(data, cb)
    DynCore.Functions.TriggerCallback('dyn-inventory:server:attemptPurchase', function(canPurchase)
        cb(canPurchase)
    end, data)
end)
...
RegisterNUICallback('CloseInventory', function(data, cb)
    ...
    if data.name then
        ...
        TriggerServerEvent('dyn-inventory:server:closeInventory', data.name)
    elseif CurrentDrop then
        TriggerServerEvent('dyn-inventory:server:closeInventory', CurrentDrop)
        CurrentDrop = nil
    end
    cb('ok')
end)
...
RegisterNUICallback('UseItem', function(data, cb)
    TriggerServerEvent('dyn-inventory:server:useItem', data.item)
    cb('ok')
end)

RegisterNUICallback('SetInventoryData', function(data, cb)
    TriggerServerEvent('dyn-inventory:server:SetInventoryData', data.fromInventory, data.toInventory, data.fromSlot, data.toSlot, data.fromAmount, data.toAmount)
    cb('ok')
end)
...
RegisterNUICallback('GiveItem', function(data, cb)
    ...
    if player ~= -1 and distance < 3 then
        ...
        DynCore.Functions.TriggerCallback('dyn-inventory:server:giveItem', function(success)
            cb(success)
        end, playerId, data.item.name, data.amount)
    else
        DynCore.Functions.Notify(Lang:t('notify.nonb'), 'error')
        cb(false)
    end
end)
...
RegisterNUICallback('RemoveAttachment', function(data, cb)
    ...
    local allAttachments = exports['dyn-weapons']:getConfigWeaponAttachments()
    local Attachment = allAttachments[data.AttachmentData.attachment][WeaponData.name]
    local itemInfo = DynCore.Shared.Items[data.AttachmentData.attachment]
    DynCore.Functions.TriggerCallback('dyn-weapons:server:RemoveAttachment', function(NewAttachments)
...
CreateThread(function()
    exports['dyn-target']:AddTargetModel(Config.VendingObjects, {
        options = {
            {
                type = 'server',
                event = 'dyn-inventory:server:openVending',
...
for i = 1, 5 do
    RegisterCommand('slot_' .. i, function()
        ...
        if itemData.type == "weapon" then
            if holdingDrop then
                return DynCore.Functions.Notify("Your already holding a bag, Go Drop it!", "error", 5500)
            end
        end
        TriggerServerEvent('dyn-inventory:server:useItem', itemData)
    end, false)
