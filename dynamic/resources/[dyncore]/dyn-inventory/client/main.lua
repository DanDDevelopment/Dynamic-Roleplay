...
RegisterNUICallback('showBlur', function()
    Wait(50)
    TriggerEvent("dyn-inventory:client:showBlur")
end) 
RegisterNetEvent("dyn-inventory:client:showBlur", function()
    Wait(50)
    ...
    if itemdata.info.attachments ~= nil and next(itemdata.info.attachments) ~= nil then
        for _, v in pairs(itemdata.info.attachments) do
            attachments[#attachments+1] = {
                attachment = v.item,
                label = v.label,
                image = DynCore.Shared.Items[v.item].image,
...
local function ItemsToItemInfo()
	...
	for i = 1, item_count do
		...
		for key, value in pairs(Config.CraftingItems[i].costs) do
			ex_string = ex_string .. DynCore.Shared.Items[key]["label"] .. ": " .. value .. "x "
		end
...
local function CreateItemDrop(index)
	...
	if Config.UseTarget then
		exports['dyn-target']:AddTargetEntity(dropItem, {
			options = {
				{
...
RegisterNetEvent('DynCore:Client:OnPlayerLoaded', function()
    ...
    DynCore.Functions.TriggerCallback("inventory:server:GetCurrentDrops", function(theDrops)
		Drops = theDrops
    end)
end)
...
RegisterNetEvent('inventory:client:CheckOpenState', function(type, id, label)
    local name = DynCore.Shared.SplitStr(label, "-")[2]
    if type == "stash" then
    ...
    if bool then
        for k in pairs(items) do
            itemTable[#itemTable+1] = {
                item = items[k].name,
                label = DynCore.Shared.Items[items[k].name]["label"],
                image = items[k].image,
            }
        end
    end
...
RegisterNetEvent('inventory:client:OpenInventory', function(PlayerAmmo, inventory, other)
    if not IsEntityDead(PlayerPedId()) then
        if Config.Progressbar.Enable then
            DynCore.Functions.Progressbar('open_inventory', 'Opening Inventory...', math.random(Config.Progressbar.minT, Config.Progressbar.maxT), false, true, { -- Name | Label | Time | useWhileDead | canCancel
                ...
                DynCore.Functions.TriggerCallback('inventory:server:ConvertQuality', function(data)
...
RegisterNetEvent('inventory:client:CraftItems', function(itemName, itemCosts, amount, toSlot, points)
    ...
    isCrafting = true
    DynCore.Functions.Progressbar("repair_vehicle", "Crafting..", (math.random(2000, 5000) * amount), false, true, {
        ...
        TriggerEvent('inventory:client:ItemBox', DynCore.Shared.Items[itemName], 'add')
        isCrafting = false
	end, function() -- Cancel
		StopAnimTask(ped, "mini@repair", "fixing_a_player", 1.0)
        DynCore.Functions.Notify("Failed", "error")
        isCrafting = false
	end)
end)
...
RegisterNetEvent('inventory:client:PickupSnowballs', function()
    ...
    DynCore.Functions.Progressbar("pickupsnowball", "Collecting snowballs..", 1500, false, true, {
        disableMovement = true,
        disableCarMovement = true,
        disableMouse = false,
        disableCombat = true,
        ...
        TriggerEvent('inventory:client:ItemBox', DynCore.Shared.Items["snowball"], "add")
    end, function() -- Cancel
        ClearPedTasks(ped)
        DynCore.Functions.Notify("Canceled", "error")
    end)
end)
...
RegisterNetEvent("dyn-inventory:client:closeinv", function()
    closeInventory()
end)
    ...
    if not isCrafting and not inInventory then
        if not PlayerData.metadata["isdead"] and not PlayerData.metadata["inlaststand"] and not PlayerData.metadata["ishandcuffed"] and not IsPauseMenuActive() then
            ...
            if IsPedInAnyVehicle(ped, false) then -- Is Player In Vehicle
                ...
                CurrentGlovebox = DynCore.Functions.GetPlate(vehicle)
                curVeh = vehicle
                CurrentVehicle = nil
            else
                local vehicle = DynCore.Functions.GetClosestVehicle()
                if vehicle ~= 0 and vehicle ~= nil then
                    ...
                    if #(pos - trunkpos) < 1.5 and not IsPedInAnyVehicle(ped) then
                        if GetVehicleDoorLockStatus(vehicle) < 2 then
                            CurrentVehicle = DynCore.Functions.GetPlate(vehicle)
                            curVeh = vehicle
                            CurrentGlovebox = nil
                        else
                            DynCore.Functions.Notify("Vehicle locked.", "error")
                            return
                        end
                    else
                        CurrentVehicle = nil
...
RegisterNetEvent('dyn-inventory:client:giveAnim', function()
...
RegisterNUICallback('Notify', function(data, cb)
    DynCore.Functions.Notify(data.message, data.type)
    cb('ok')
end)

RegisterNUICallback('GetWeaponData', function(cData, cb)
    local data = {
        WeaponData = DynCore.Shared.Items[cData.weapon],
...
RegisterNUICallback('RemoveAttachment', function(data, cb)
    local ped = PlayerPedId()
    local WeaponData = DynCore.Shared.Items[data.WeaponData.name]
    print(data.AttachmentData.attachment:gsub("(.*).*_",''))
    data.AttachmentData.attachment = data.AttachmentData.attachment:gsub("(.*).*_",'')
    DynCore.Functions.TriggerCallback('weapons:server:RemoveAttachment', function(NewAttachments)
...
RegisterNUICallback('getCombineItem', function(data, cb)
    cb(DynCore.Shared.Items[data.item])
end)
...
RegisterNUICallback('combineWithAnim', function(data, cb)
    ...
    DynCore.Functions.Progressbar("combine_anim", animText, animTimeout, false, true, {
        disableMovement = false,
        disableCarMovement = true,
        disableMouse = false,
        disableCombat = true,
...
RegisterNUICallback("GiveItem", function(data, cb)
    local player, distance = DynCore.Functions.GetClosestPlayer(GetEntityCoords(PlayerPedId()))
    if player ~= -1 and distance < 3 then
        if data.inventory == 'player' then
            ...
            DynCore.Functions.Notify("You do not own this item!", "error")
        end
    else
...
CreateThread(function()
    ...
    exports['dyn-target']:AddTargetModel(toolBoxModels, {
            options = {
                {
