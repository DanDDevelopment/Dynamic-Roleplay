...
function GetDrops()
    DynCore.Functions.TriggerCallback('dyn-inventory:server:GetCurrentDrops', function(drops)
        if drops then
            ...
            },
            {
                ...
                action = function()
                    if IsPedArmed(PlayerPedId(), 4) then
                        return DynCore.Functions.Notify("You can not be holding a Gun and a Bag!", "error", 5500) 
                    end
                    if holdingDrop then
...
RegisterNUICallback('DropItem', function(item, cb)
    DynCore.Functions.TriggerCallback('dyn-inventory:server:createDrop', function(dropId)
        if dropId then
            while not NetworkDoesNetworkIdExist(dropId) do Wait(10) end
            local bag = NetworkGetEntityFromNetworkId(dropId)
