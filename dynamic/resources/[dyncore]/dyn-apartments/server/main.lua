...
RegisterNetEvent('dyn-apartments:server:SetInsideMeta', function(house, insideId, bool, isVisiting)
    local src = source
    ...
    if bool then
        ...
        DynCore.Functions.SetPlayerBucket(src, tonumber(routeId))
    else
        ...
        Player.Functions.SetMetaData('inside', insideMeta)
        DynCore.Functions.SetPlayerBucket(src, 0)
    end
end)

RegisterNetEvent('dyn-apartments:returnBucket', function()
    local src = source
...
RegisterNetEvent('apartments:server:openStash', function(CurrentApartment)
    local src = source
    exports['dyn-inventory']:OpenInventory(src, CurrentApartment)
end)
...
RegisterNetEvent('apartments:server:CreateApartment', function(type, label)
    ...
    TriggerClientEvent('DynCore:Notify', src, Lang:t('success.receive_apart') .. ' (' .. label .. ')')
...
RegisterNetEvent('apartments:server:UpdateApartment', function(type, label)
    ...
    TriggerClientEvent('DynCore:Notify', src, Lang:t('success.changed_apart'))
    TriggerClientEvent('apartments:client:SetHomeBlip', src, type)
end)
...
DynCore.Functions.CreateCallback('apartments:GetAvailableApartments', function(_, cb, apartment)
    local apartments = {}
...
DynCore.Functions.CreateCallback('apartments:GetApartmentOffset', function(_, cb, apartmentId)
    local retval = 0
...
DynCore.Functions.CreateCallback('apartments:GetApartmentOffsetNewOffset', function(_, cb, apartment)
...
DynCore.Functions.CreateCallback('apartments:GetOwnedApartment', function(source, cb, cid)
    if cid ~= nil then
...
DynCore.Functions.CreateCallback('apartments:IsOwner', function(source, cb, apartment)
    local src = source
    local Player = DynCore.Functions.GetPlayer(src)
    if Player ~= nil then
        local result = MySQL.query.await('SELECT * FROM apartments WHERE citizenid = ?', { Player.PlayerData.citizenid })
...
DynCore.Functions.CreateCallback('apartments:GetOutfits', function(source, cb)
    local src = source
    local Player = DynCore.Functions.GetPlayer(src)
