...
DynCore.Functions.CreateCallback('dyn-jewellery:server:getCops', function(source, cb)
    local amount = 0
...
DynCore.Functions.CreateCallback('dyn-jewellery:server:getVitrineState', function(_, cb)
    cb(Config.Locations)
end)
        ...
        if plrPed then
            ...
            if dist <= 25.0 then
                ...
                if exports['dyn-inventory']:AddItem(src, Config.VitrineRewards[item]['item'], amount, false, false, 'dyn-jewellery:server:vitrineReward') then
                    TriggerClientEvent('dyn-inventory:client:ItemBox', src, DynCore.Shared.Items[Config.VitrineRewards[item]['item']], 'add')
                else
                    TriggerClientEvent('DynCore:Notify', src, Lang:t('error.to_much'), 'error')
                end
            else
                cheating = true
            end
