...
RegisterNetEvent('hospital:server:SendToBed', function(bedId, isRevive, hospitalIndex)
	...
	exports['dyn-banking']:AddMoney('ambulance', Config.BillCost, 'Player treatment')
		...
		for i = 1, #Config.Locations['jailbeds'] do
			...
			if not v.taken then
				...
				if Config.WipeInventoryOnRespawn then
					...
					TriggerClientEvent('DynCore:Notify', src, Lang:t('error.possessions_taken'), 'error')
				end
				Player.Functions.RemoveMoney('bank', Config.BillCost, 'respawned-at-hospital')
				exports['dyn-banking']:AddMoney('ambulance', Config.BillCost, 'Player treatment')
...
RegisterNetEvent('hospital:server:TreatWounds', function(playerId)
	local src = source
	local Player = DynCore.Functions.GetPlayer(src)
	local Patient = DynCore.Functions.GetPlayer(playerId)
	if Patient then
		if Player.PlayerData.job.name == 'ambulance' then
			exports['dyn-inventory']:RemoveItem(src, 'bandage', 1, false, 'hospital:server:TreatWounds')
			TriggerClientEvent('dyn-inventory:client:ItemBox', src, DynCore.Shared.Items['bandage'], 'remove')
...
RegisterNetEvent('hospital:server:RevivePlayer', function(playerId, isOldMan)
	...
	if Patient then
		if Player.PlayerData.job.name == 'ambulance' or DynCore.Functions.HasItem(src, 'firstaid', 1) then
			if oldMan then
				if Player.Functions.RemoveMoney('cash', 5000, 'revived-player') then
					exports['dyn-inventory']:RemoveItem(src, 'firstaid', 1, false, 'hospital:server:RevivePlayer')
					TriggerClientEvent('dyn-inventory:client:ItemBox', src, DynCore.Shared.Items['firstaid'], 'remove')
			...
			TriggerEvent('dyn-log:server:CreateLog', 'ambulancejob', 'Player Banned', 'red', string.format('%s was banned by %s for %s', GetPlayerName(src), 'dyn-ambulancejob', 'Trying to revive theirselves or other players'), true)
...
RegisterNetEvent('hospital:server:SendDoctorAlert', function(hospitalName)
	...
	if not doctorCalled then
		...
		for _, v in pairs(players) do
			if v.PlayerData.job.name == 'ambulance' and v.PlayerData.job.onduty then
				TriggerClientEvent('DynCore:Notify', v.PlayerData.source, Lang:t('info.dr_needed', { hospital = hospitalName }), 'ambulance')
			end
		end
		...
		TriggerClientEvent('DynCore:Notify', src, 'Doctor has already been notified', 'error')
	end
end)

RegisterNetEvent('hospital:server:UseFirstAid', function(targetId)
	local src = source
	local Target = DynCore.Functions.GetPlayer(targetId)
	if Target then
...
RegisterNetEvent('hospital:server:CanHelp', function(helperId, canHelp)
	...
	if canHelp then
		...
		TriggerClientEvent('DynCore:Notify', helperId, Lang:t('error.cant_help'), 'error')
	end
end)
...
RegisterNetEvent('hospital:server:removeBandage', function()
	...
	exports['dyn-inventory']:RemoveItem(source, 'bandage', 1, false, 'hospital:server:removeBandage')
end)

RegisterNetEvent('hospital:server:removeIfaks', function()
	local Player = DynCore.Functions.GetPlayer(source)
	if not Player then return end
	exports['dyn-inventory']:RemoveItem(source, 'ifaks', 1, false, 'hospital:server:removeIfaks')
end)

RegisterNetEvent('hospital:server:removePainkillers', function()
	local Player = DynCore.Functions.GetPlayer(source)
	if not Player then return end
	exports['dyn-inventory']:RemoveItem(source, 'painkillers', 1, false, 'hospital:server:removePainkillers')
end)
...
RegisterNetEvent('dyn-ambulancejob:server:stash', function()
	local src = source
	local Player = DynCore.Functions.GetPlayer(src)
	if not Player then return end
...
DynCore.Functions.CreateCallback('hospital:GetDoctors', function(_, cb)
	local amount = 0
	local players = DynCore.Functions.GetQBPlayers()
	for _, v in pairs(players) do
		if v.PlayerData.job.name == 'ambulance' and v.PlayerData.job.onduty then
...
DynCore.Functions.CreateCallback('hospital:GetPlayerStatus', function(_, cb, playerId)
...
DynCore.Functions.CreateCallback('hospital:GetPlayerBleeding', function(source, cb)
	local src = source
...
DynCore.Commands.Add('911e', Lang:t('info.ems_report'), { { name = 'message', help = Lang:t('info.message_sent') } }, false, function(source, args)
	local src = source
	local message
...
DynCore.Commands.Add('status', Lang:t('info.check_health'), {}, false, function(source, _)
	local src = source
	local Player = DynCore.Functions.GetPlayer(src)
	if Player.PlayerData.job.name == 'ambulance' then
		TriggerClientEvent('hospital:client:CheckStatus', src)
	else
		TriggerClientEvent('DynCore:Notify', src, Lang:t('error.not_ems'), 'error')
	end
end)

DynCore.Commands.Add('heal', Lang:t('info.heal_player'), {}, false, function(source, _)
	local src = source
	local Player = DynCore.Functions.GetPlayer(src)
	if Player.PlayerData.job.name == 'ambulance' then
...
DynCore.Commands.Add('revivep', Lang:t('info.revive_player'), {}, false, function(source, _)
	local src = source
	local Player = DynCore.Functions.GetPlayer(src)
	if Player.PlayerData.job.name == 'ambulance' then
...
DynCore.Commands.Add('revive', Lang:t('info.revive_player_a'), { { name = 'id', help = Lang:t('info.player_id') } }, false, function(source, args)
	local src = source
	if args[1] then
		...
		if Player then
			...
			TriggerClientEvent('DynCore:Notify', src, Lang:t('error.not_online'), 'error')
		end
	else
...
DynCore.Commands.Add('setpain', Lang:t('info.pain_level'), { { name = 'id', help = Lang:t('info.player_id') } }, false, function(source, args)
	local src = source
	if args[1] then
		local Player = DynCore.Functions.GetPlayer(tonumber(args[1]))
		if Player then
...
DynCore.Commands.Add('kill', Lang:t('info.kill'), { { name = 'id', help = Lang:t('info.player_id') } }, false, function(source, args)
	local src = source
	if args[1] then
		local Player = DynCore.Functions.GetPlayer(tonumber(args[1]))
		if Player then
...
DynCore.Commands.Add('aheal', Lang:t('info.heal_player_a'), { { name = 'id', help = Lang:t('info.player_id') } }, false, function(source, args)
	local src = source
	if args[1] then
		local Player = DynCore.Functions.GetPlayer(tonumber(args[1]))
		if Player then
...
DynCore.Functions.CreateUseableItem('ifaks', function(source, item)
	local src = source
	local Player = DynCore.Functions.GetPlayer(src)
...
DynCore.Functions.CreateUseableItem('bandage', function(source, item)
	local src = source
	local Player = DynCore.Functions.GetPlayer(src)
	if Player.Functions.GetItemByName(item.name) ~= nil then
...
DynCore.Functions.CreateUseableItem('painkillers', function(source, item)
	local src = source
	local Player = DynCore.Functions.GetPlayer(src)
	if Player.Functions.GetItemByName(item.name) ~= nil then
...
DynCore.Functions.CreateUseableItem('firstaid', function(source, item)
	local src = source
	local Player = DynCore.Functions.GetPlayer(src)
	if Player.Functions.GetItemByName(item.name) ~= nil then
