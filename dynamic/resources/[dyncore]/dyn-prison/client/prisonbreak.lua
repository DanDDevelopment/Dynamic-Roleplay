...
RegisterNetEvent('electronickit:UseElectronickit', function()
    if currentGate ~= 0 and not securityLockdown and not Gates[currentGate].hit then
        local hasItem = DynCore.Functions.HasItem('gatecrack')
        if hasItem then
            TriggerEvent('dyn-inventory:client:requiredItems', requiredItems, false)
            DynCore.Functions.Progressbar('hack_gate', Lang:t('info.connecting_device'), math.random(5000, 10000), false, true, {
                ...
                if success then
                    ...
                    TriggerServerEvent('dyn-doorlock:server:updateState', Gates[currentGate].gatekey, false, false, false, true)
                else
                    TriggerServerEvent('prison:server:SecurityLockdown')
                end
            end, function() -- Cancel
                StopAnimTask(PlayerPedId(), 'anim@gangops@facility@servers@', 'hotwire', 1.0)
                DynCore.Functions.Notify(Lang:t('error.cancelled'), 'error')
            end)
        else
            DynCore.Functions.Notify(Lang:t('error.item_missing'), 'error')
        end
    end
end)
...
RegisterNetEvent('prison:client:PrisonBreakAlert', function()
    ...
    TriggerEvent('dyn-phone:client:addPoliceAlert', alertData)
...
CreateThread(function()
    Wait(500)
    requiredItems = {
        [1] = { name = DynCore.Shared.Items['electronickit']['name'], image = DynCore.Shared.Items['electronickit']['image'] },
        [2] = { name = DynCore.Shared.Items['gatecrack']['name'], image = DynCore.Shared.Items['gatecrack']['image'] },
    }
    while true do
        inRange = false
        currentGate = 0
            ...
            for k in pairs(Gates) do
                ...
                if dist < 1.5 then
                    ...
                    elseif not requiredItemsShowed then
                        ...
                        TriggerEvent('dyn-inventory:client:requiredItems', requiredItems, true)
                    end
                end
            end
...
CreateThread(function()
    while true do
        ...
        if #(pos.xy - vec2(Config.Locations['middle'].x, Config.Locations['middle'].y)) > 200 and inJail then
            ...
            DynCore.Functions.Notify(Lang:t('error.escaped'), 'error')
        end
        Wait(1000)
    end
end)
