...
local function SaveSkin()
    ...
    TriggerServerEvent("dyn-clothing:saveSkin", model, clothing)
end
...
local function reloadSkin(health)
    ...
    local gender = DynCore.Functions.GetPlayerData().charinfo.gender
    ...
    TriggerServerEvent("dyn-clothing:loadPlayerSkin") -- LOADING PLAYER'S CLOTHES - Event 2
...
local function getOutfits(gradeLevel, data)
    local gender = "male"
    if DynCore.Functions.GetPlayerData().charinfo.gender == 1 then gender = "female" end
    DynCore.Functions.TriggerCallback('dyn-clothing:server:getOutfits', function(result)
        openMenu({
...
RegisterNetEvent('dyn-clothing:client:openMenu', function()
...
RegisterNetEvent('dyn-clothing:client:reloadOutfits', function(myOutfits)
    SendNUIMessage({
        action = "reloadMyOutfits",
        outfits = myOutfits
    })
end)
RegisterNetEvent('dyn-clothes:client:CreateFirstCharacter', function()
    DynCore.Functions.GetPlayerData(function(pData)
...
RegisterNetEvent("dyn-clothes:loadSkin", function(_, model, data)
    ...
    Citizen.CreateThread(function()
        ...
        TriggerEvent('dyn-clothing:client:loadPlayerClothing', data, PlayerPedId())
    end)
end)

RegisterNetEvent('dyn-clothing:client:loadPlayerClothing', function(data, ped)
...
RegisterNetEvent('dyn-clothing:client:loadOutfit', function(oData)
    local ped = PlayerPedId()
    ...
    if data["accessory"] ~= nil then
        if DynCore.Functions.GetPlayerData().metadata["tracker"] then
    ...
    if oData.outfitName ~= nil then
        DynCore.Functions.Notify("You have chosen "..oData.outfitName.."! Press Confirm to confirm outfit.")
    end
end)
RegisterNetEvent("dyn-clothing:client:adjustfacewear", function(type)
    if DynCore.Functions.GetPlayerData().metadata["ishandcuffed"] then return end
...
RegisterNetEvent('DynCore:Client:OnPlayerLoaded', function()
    TriggerServerEvent("dyn-clothes:loadPlayerSkin")
    PlayerData = DynCore.Functions.GetPlayerData()
    loadStores()
--    DynCore.Shared.Jobs = exports['dyn-jobs']:AddJobs()
end)
...
RegisterNetEvent('dyn-clothing:client:openOutfitMenu', function()
    DynCore.Functions.TriggerCallback('dyn-clothing:server:getOutfits', function(result)
        openMenu({
...
RegisterNUICallback('selectOutfit', function(data, cb)
    TriggerEvent('dyn-clothing:client:loadOutfit', data)
    cb('ok')
end)
...
RegisterNUICallback('TrackerError', function(_, cb)
    DynCore.Functions.Notify(Lang:t("notify.error_bracelet"), "error")
    cb('ok')
end)
RegisterNUICallback('saveOutfit', function(data, cb)
    local ped = PlayerPedId()
    local model = GetEntityModel(ped)
    TriggerServerEvent('dyn-clothes:saveOutfit', data.outfitName, model, skinData)
    cb('ok')
end)
...
RegisterNUICallback('close', function(_, cb)
    ...
    disableCam()
    FreezeEntityPosition(PlayerPedId(), false)
    TriggerEvent('dyn-clothing:client:onMenuClose')
    cb('ok')
end)
...
RegisterNUICallback('removeOutfit', function(data, cb)
    TriggerServerEvent('dyn-clothing:server:removeOutfit', data.outfitName, data.outfitId)
    DynCore.Functions.Notify(Lang:t('notify.info_deleteOutfit', {outfit = data.outfitName}))
    cb('ok')
end)
    ...
    if Config.UseTarget then
        CreateThread(function()
            for k, v in pairs(Config.Stores) do
                ...
                exports['dyn-target']:AddBoxZone(v.shopType .. k, v.coords, v.length, v.width, {
                    name = v.shopType .. k,
                    debugPoly = false,
                    minZ = v.coords.z-1,
                    maxZ = v.coords.z+1,
            ...
            for k, v in pairs(Config.ClothingRooms) do
                ...
                elseif not DynCore.Shared.QBJobsStatus then
                    action = function()
                        customCamLocation = v.cameraLocation
                ...
                exports['dyn-target']:AddBoxZone('clothing_' .. v.requiredJob .. k, v.coords, v.length, v.width, {
            ...
            clothingCombo:onPlayerInOut(function(isPointInside, _, zone)
                if isPointInside then
                    ...
                    if zoneName == 'surgeon' then
                        exports['dyn-core']:DrawText('[E] - '..Lang:t("store.surgeon"), 'left')
                    elseif zoneName == 'clothing' then
                        exports['dyn-core']:DrawText('[E] - '..Lang:t("store.clothing"), 'left')
                    elseif zoneName == 'barber' then
                        exports['dyn-core']:DrawText('[E] - '..Lang:t("store.barber"), 'left')
                    elseif zoneName == 'outfit' then
                        exports['dyn-core']:DrawText('[E] - '..Lang:t("store.outfitchanger"), 'left')
                    end
                else
                    inZone = false
                    ...
                    if PlayerData.gang and PlayerData.gang.name or (not DynCore.Shared.QBJobsStatus and PlayerData.job.name) then
                        ...
                        clothingRoomsCombo:onPlayerInOut(function(isPointInside, _, zone)
                            if isPointInside then
                                local zoneID = tonumber(DynCore.Shared.SplitStr(zone.name, "_")[2])
                                local job = Config.ClothingRooms[zoneID].isGang and PlayerData.gang.name or (not DynCore.Shared.QBJobsStatus and PlayerData.job.name)
                ...
                if inZone then
                    ...
                    elseif not DynCore.Shared.QBJobsStatus then
                        if IsControlJustReleased(0, 38) then
                            ...
                            local gradeLevel = clothingRoom.isGang and PlayerData.gang.grade.level or (not DynCore.Shared.QBJobsStatus and PlayerData.job.grade.level)
