...
CreateThread(function()
    ...
    TriggerClientEvent('dyn-garages:client:houseGarageConfig', -1, HouseGarages)
    TriggerClientEvent('dyn-houses:client:setHouseConfig', -1, Config.Houses)
end)
...
DynCore.Commands.Add('decorate', Lang:t('info.decorate_interior'), {}, false, function(source)
    local src = source
    TriggerClientEvent('dyn-houses:client:decorate', src)
end)

DynCore.Commands.Add('createhouse', Lang:t('info.create_house'), { { name = 'price', help = Lang:t('info.price_of_house') }, { name = 'tier', help = Lang:t('info.tier_number') } }, true, function(source, args)
    local src = source
    local Player = DynCore.Functions.GetPlayer(src)
    local price = tonumber(args[1])
    local tier = tonumber(args[2])
    if Player.PlayerData.job.name == 'realestate' then
        TriggerClientEvent('dyn-houses:client:createHouses', src, price, tier)
    else
        TriggerClientEvent('DynCore:Notify', src, Lang:t('error.realestate_only'), 'error')
    end
end)

DynCore.Commands.Add('addgarage', Lang:t('info.add_garage'), {}, false, function(source)
    local src = source
    local Player = DynCore.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == 'realestate' then
        TriggerClientEvent('dyn-houses:client:addGarage', src)
    else
        TriggerClientEvent('DynCore:Notify', src, Lang:t('error.realestate_only'), 'error')
    end
end)

DynCore.Commands.Add('ring', Lang:t('info.ring_doorbell'), {}, false, function(source)
    local src = source
    TriggerClientEvent('dyn-houses:client:RequestRing', src)
end)

-- Item

DynCore.Functions.CreateUseableItem('police_stormram', function(source, _)
    local Player = DynCore.Functions.GetPlayer(source)
    if (Player.PlayerData.job.name == 'police' and Player.PlayerData.job.onduty) then
        TriggerClientEvent('dyn-houses:client:HomeInvasion', source)
    else
        TriggerClientEvent('DynCore:Notify', source, Lang:t('error.emergency_services'), 'error')
    end
end)
...
RegisterNetEvent('dyn-houses:server:setHouses', function()
    local src = source
    TriggerClientEvent('dyn-houses:client:setHouseConfig', src, Config.Houses)
end)

RegisterNetEvent('dyn-houses:server:createBlip', function()
    local src = source
    local ped = GetPlayerPed(src)
    local coords = GetEntityCoords(ped)
    TriggerClientEvent('dyn-houses:client:createBlip', -1, coords)
end)

RegisterNetEvent('dyn-houses:server:addNewHouse', function(street, coords, price, tier)
    local src = source
    street = street:gsub("%'", '')
    ...
    TriggerClientEvent('dyn-houses:client:setHouseConfig', -1, Config.Houses)
    TriggerClientEvent('DynCore:Notify', src, Lang:t('info.added_house', { value = label }))
    TriggerEvent('dyn-log:server:CreateLog', 'house', Lang:t('log.house_created'), 'green', Lang:t('log.house_address', { label = label, price = price, tier = tier, agent = GetPlayerName(src) }))
end)

RegisterNetEvent('dyn-houses:server:addGarage', function(house, coords)
    local src = source
    ...
    TriggerClientEvent('dyn-garages:client:addHouseGarage', -1, house, garageInfo)
    TriggerClientEvent('DynCore:Notify', src, Lang:t('info.added_garage', { value = garageInfo.label }))
end)

RegisterNetEvent('dyn-houses:server:viewHouse', function(house)
    local src = source
    local pData = DynCore.Functions.GetPlayer(src)
    ...
    TriggerClientEvent('dyn-houses:client:viewHouse', src, houseprice, brokerfee, bankfee, taxes, pData.PlayerData.charinfo.firstname, pData.PlayerData.charinfo.lastname)
end)

RegisterNetEvent('dyn-houses:server:openStash', function(CurrentHouse)
    local src = source
    ...
    if stashSlots and stashWeight then
        exports['dyn-inventory']:OpenInventory(src, CurrentHouse, {
...
RegisterNetEvent('dyn-houses:server:buyHouse', function(house)
    local src = source
    local pData = DynCore.Functions.GetPlayer(src)
    ...
    if isOwned then
        TriggerClientEvent('DynCore:Notify', src, Lang:t('error.already_owned'), 'error')
        CancelEvent()
        return
    end
    ...
    if (bankBalance >= HousePrice) then
        ...
        TriggerClientEvent('dyn-houses:client:SetClosestHouse', src)
        TriggerClientEvent('dyn-house:client:RefreshHouseTargets', src)
        pData.Functions.RemoveMoney('bank', HousePrice, 'bought-house') -- 21% Extra house costs
        exports['dyn-banking']:AddMoney('realestate', (HousePrice / 100) * math.random(18, 25), 'House purchase')
        TriggerEvent('dyn-log:server:CreateLog', 'house', Lang:t('log.house_purchased'), 'green', Lang:t('log.house_purchased_by', { house = house:upper(), price = HousePrice, firstname = pData.PlayerData.charinfo.firstname, lastname = pData.PlayerData.charinfo.lastname }))
        TriggerClientEvent('DynCore:Notify', src, Lang:t('success.house_purchased'), 'success', 5000)
    else
        TriggerClientEvent('DynCore:Notify', src, Lang:t('error.not_enough_money'), 'error')
    end
end)

RegisterNetEvent('dyn-houses:server:lockHouse', function(bool, house)
    TriggerClientEvent('dyn-houses:client:lockHouse', -1, bool, house)
end)

RegisterNetEvent('dyn-houses:server:SetRamState', function(bool, house)
    Config.Houses[house].IsRaming = bool
    TriggerClientEvent('dyn-houses:server:SetRamState', -1, bool, house)
end)

RegisterNetEvent('dyn-houses:server:giveKey', function(house, target)
    local src = source
    local Player = DynCore.Functions.GetPlayer(src)
    local pData = DynCore.Functions.GetPlayer(target)
    ...
    if not isHouseOwner(Player.PlayerData.license, Player.PlayerData.citizenid, house) then
        TriggerClientEvent('DynCore:Notify', src, Lang:t('error.not_owner'), 'error')
        return
    end
...
RegisterNetEvent('dyn-houses:server:removeHouseKey', function(house, citizenData)
    local src = source
    local Player = DynCore.Functions.GetPlayer(src)
    ...
    TriggerClientEvent('DynCore:Notify', src, Lang:t('error.remove_key_from', { firstname = citizenData.firstname, lastname = citizenData.lastname }), 'error')
    MySQL.update('UPDATE player_houses SET keyholders = ? WHERE house = ?', { json.encode(housekeyholders[house]), house })
end)

RegisterNetEvent('dyn-houses:server:OpenDoor', function(target, house)
    local OtherPlayer = DynCore.Functions.GetPlayer(target)
    if OtherPlayer then
        TriggerClientEvent('dyn-houses:client:SpawnInApartment', OtherPlayer.PlayerData.source, house)
    end
end)

RegisterNetEvent('dyn-houses:server:RingDoor', function(house)
    local src = source
    TriggerClientEvent('dyn-houses:client:RingDoor', -1, src, house)
end)

RegisterNetEvent('dyn-houses:server:savedecorations', function(house, decorations)
    MySQL.update('UPDATE player_houses SET decorations = ? WHERE house = ?', { json.encode(decorations), house })
    TriggerClientEvent('dyn-houses:server:sethousedecorations', -1, house, decorations)
end)

RegisterNetEvent('dyn-houses:server:LogoutLocation', function()
    local src = source
    local Player = DynCore.Functions.GetPlayer(src)
...
RegisterNetEvent('dyn-houses:server:giveHouseKey', function(target, house)
    local src = source
    local Player = DynCore.Functions.GetPlayer(src)
    local tPlayer = DynCore.Functions.GetPlayer(target)
    ...
    if housekeyholders[house] then
        for _, cid in pairs(housekeyholders[house]) do
            if cid == tPlayer.PlayerData.citizenid then
                TriggerClientEvent('DynCore:Notify', src, Lang:t('error.already_keys'), 'error', 3500)
                return
            end
        end
        housekeyholders[house][#housekeyholders[house] + 1] = tPlayer.PlayerData.citizenid
        MySQL.update('UPDATE player_houses SET keyholders = ? WHERE house = ?', { json.encode(housekeyholders[house]), house })
        TriggerClientEvent('dyn-houses:client:refreshHouse', tPlayer.PlayerData.source)

        TriggerClientEvent('DynCore:Notify', tPlayer.PlayerData.source, Lang:t('success.recieved_key', { value = Config.Houses[house].adress }), 'success', 2500)
    else
        local sourceTarget = DynCore.Functions.GetPlayer(src)
        housekeyholders[house] = {
...
RegisterNetEvent('dyn-houses:server:setLocation', function(coords, house, type)
    if type == 1 then
    ...
    TriggerClientEvent('dyn-houses:client:refreshLocations', -1, house, json.encode(coords), type)
end)

RegisterNetEvent('dyn-houses:server:SetHouseRammed', function(bool, house)
    Config.Houses[house].IsRammed = bool
    TriggerClientEvent('dyn-houses:client:SetHouseRammed', -1, bool, house)
end)

RegisterNetEvent('dyn-houses:server:SetInsideMeta', function(insideId, bool)
    local src = source
    local Player = DynCore.Functions.GetPlayer(src)
...
DynCore.Functions.CreateCallback('dyn-houses:server:buyFurniture', function(source, cb, price)
    local src = source
    local pData = DynCore.Functions.GetPlayer(src)
    local bankBalance = pData.PlayerData.money['bank']
...
DynCore.Functions.CreateCallback('dyn-houses:server:ProximityKO', function(source, cb, house)
    local src = source
    local Player = DynCore.Functions.GetPlayer(src)
    local retvalK = false
    local retvalO
...
DynCore.Functions.CreateCallback('dyn-houses:server:hasKey', function(source, cb, house)
    local src = source
    local Player = DynCore.Functions.GetPlayer(src)
    local retval = false
    if Player then
...
DynCore.Functions.CreateCallback('dyn-houses:server:isOwned', function(source, cb, house)
    local src = source
    local Player = DynCore.Functions.GetPlayer(src)
...
DynCore.Functions.CreateCallback('dyn-houses:server:getHouseOwner', function(_, cb, house)
    cb(houseownercid[house])
end)

DynCore.Functions.CreateCallback('dyn-houses:server:getHouseKeyHolders', function(source, cb, house)
    local retval = {}
    local src = source
    local Player = DynCore.Functions.GetPlayer(src)
    if housekeyholders[house] then
...
DynCore.Functions.CreateCallback('dyn-phone:server:TransferCid', function(_, cb, NewCid, house)
...
DynCore.Functions.CreateCallback('dyn-houses:server:getHouseDecorations', function(_, cb, house)
    local retval = nil
...
DynCore.Functions.CreateCallback('dyn-houses:server:getHouseLocations', function(_, cb, house)
    local retval = nil
    local result = MySQL.query.await('SELECT * FROM player_houses WHERE house = ?', { house })
    if result[1] then
        retval = result[1]
...
DynCore.Functions.CreateCallback('dyn-houses:server:getOwnedHouses', function(source, cb)
    local src = source
    local pData = DynCore.Functions.GetPlayer(src)
    if pData then
...
DynCore.Functions.CreateCallback('dyn-houses:server:getSavedOutfits', function(source, cb)
    local src = source
    local pData = DynCore.Functions.GetPlayer(src)
...
DynCore.Functions.CreateCallback('dyn-phone:server:GetPlayerHouses', function(source, cb)
    local src = source
    local Player = DynCore.Functions.GetPlayer(src)
    local MyHouses = {}
...
DynCore.Functions.CreateCallback('dyn-phone:server:GetHouseKeys', function(source, cb)
    local src = source
    local Player = DynCore.Functions.GetPlayer(src)
    local MyKeys = {}
...
DynCore.Functions.CreateCallback('dyn-phone:server:MeosGetPlayerHouses', function(_, cb, input)
    if input then
