...
local function getJobs()
    DynCore.Functions.TriggerCallback('dyn-cityhall:server:receiveJobs', function(result)
        SendNUIMessage({
            action = 'setJobs',
            jobs = result
        })
        ...
        if Config.UseTarget then
            ...
            if current.drivingschool then
                opts = {
                    ...
                    action = function()
                        TriggerServerEvent('dyn-cityhall:server:sendDriverTest', Config.DrivingSchools[closestDrivingSchool].instructors)
                    end
                }
            ...
            if opts then
                exports['dyn-target']:AddTargetEntity(ped, {
                    options = { opts },
                    distance = 2.0
                })
            end
                    ...
                    if isLoggedIn and closestCityhall and closestDrivingSchool then
                        if inside then
                            if current.drivingschool then
                                ...
                                exports['dyn-core']:DrawText('[E] Take Driving Lessons')
                            elseif current.cityhall then
                                inRangeCityhall = true
                                exports['dyn-core']:DrawText('[E] Open Cityhall')
                            end
                        else
...
RegisterNetEvent('dyn-cityhall:Client:AddCityJob', function()
    getJobs()
end)

RegisterNetEvent('dyn-cityhall:client:getIds', function()
    TriggerServerEvent('dyn-cityhall:server:getIDs')
end)

RegisterNetEvent('dyn-cityhall:client:sendDriverEmail', function(charinfo)
...
RegisterNUICallback('close', function(_, cb)
    ...
    if not Config.UseTarget and inRangeCityhall then exports['dyn-core']:DrawText('[E] Open Cityhall') end -- Reopen interaction when you're still inside the zone
    cb('ok')
end)
...
RegisterNUICallback('requestId', function(id, cb)
    ...
    if inRangeCityhall and license and id.cost == license.cost then
        TriggerServerEvent('dyn-cityhall:server:requestId', id.type, closestCityhall)
        DynCore.Functions.Notify(('You have received your %s for $%s'):format(license.label, id.cost), 'success', 3500)
    else
        DynCore.Functions.Notify(Lang:t('error.not_in_range'), 'error')
    end
    cb('ok')
end)
...
RegisterNUICallback('applyJob', function(job, cb)
    if inRangeCityhall then
        TriggerServerEvent('dyn-cityhall:server:ApplyJob', job, Config.Cityhalls[closestCityhall].coords)
    else
        DynCore.Functions.Notify(Lang:t('error.not_in_range'), 'error')
    end
    cb('ok')
