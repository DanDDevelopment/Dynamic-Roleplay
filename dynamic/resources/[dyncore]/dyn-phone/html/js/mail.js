...
$(document).on('click', '#accept-mail', function(e){
    ...
    $.post('https://dyn-phone/AcceptMailButton', JSON.stringify({
...
$(document).on('click', '#remove-mail', function(e){
    e.preventDefault();
    var MailData = $("#"+OpenedMail).data('MailData');
    $.post('https://dyn-phone/RemoveMail', JSON.stringify({
        mailId: MailData.mailid
    }));
    $(".mail-home").animate({
        left: 0+"vh"
...
QB.Phone.Functions.SetupMails = function(Mails) {
    ...
    $("#mail-header-mail").html(QB.Phone.Data.PlayerData.charinfo.firstname+"."+QB.Phone.Data.PlayerData.charinfo.lastname+"@DynCore.com");
...
$(document).on('click','#new-advert-photo',function(e){
    e.preventDefault();
    $.post('https://dyn-phone/TakePhoto',function(url){
        if(url){
...
$(document).on('click', '#new-advert-submit', function(e){
    ...
    if (Advert !== "") {
        ...
        if (!picture){
            $.post('https://dyn-phone/PostAdvert', JSON.stringify({
                message: Advert,
                url: null
            }));
        }else {
...
$(document).on('click','#adv-delete',function(e){
    e.preventDefault();
    $.post('https://dyn-phone/DeleteAdvert', function(){
        setTimeout(function(){
