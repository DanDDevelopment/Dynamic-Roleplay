...
local function GetOnlineStatus(number)
    local Target = DynCore.Functions.GetPlayerByPhone(number)
    local retval = false
    if Target ~= nil then
        retval = true
    end
...
local function sendNewMailToOffline(citizenid, mailData)
    ...
    if Player then
        ...
        if mailData.button == nil then
            ...
            TriggerClientEvent('dyn-phone:client:NewMailNotify', src, mailData)
        else
        ...
        SetTimeout(200, function()
            ...
            TriggerClientEvent('dyn-phone:client:UpdateMails', src, mails)
        end)
    else
        if mailData.button == nil then
...
DynCore.Functions.CreateCallback('dyn-phone:server:GetCallState', function(_, cb, ContactData)
    local Target = DynCore.Functions.GetPlayerByPhone(ContactData.number)
    if Target ~= nil then
...
DynCore.Functions.CreateCallback('dyn-phone:server:GetPhoneData', function(source, cb)
    local src = source
    ...
    if Player ~= nil then
        ...
        if invoices[1] ~= nil then
            for _, v in pairs(invoices) do
                local Ply = DynCore.Functions.GetPlayerByCitizenId(v.sender)
                if Ply ~= nil then
...
DynCore.Functions.CreateCallback('dyn-phone:server:PayInvoice', function(source, cb, society, amount, invoiceId, sendercitizenid)
    local Invoices = {}
    local Ply = DynCore.Functions.GetPlayer(source)
    local SenderPly = DynCore.Functions.GetPlayerByCitizenId(sendercitizenid)
    ...
    exports['dyn-phone']:sendNewMailToOffline(sendercitizenid, invoiceMailData)
    exports['dyn-banking']:AddMoney(society, amount, 'Phone invoice')
...
DynCore.Functions.CreateCallback('dyn-phone:server:DeclineInvoice', function(source, cb, _, _, invoiceId)
    local Invoices = {}
    local Ply = DynCore.Functions.GetPlayer(source)
    MySQL.query('DELETE FROM phone_invoices WHERE id = ?', { invoiceId })
    local invoices = MySQL.query.await('SELECT * FROM phone_invoices WHERE citizenid = ?', { Ply.PlayerData.citizenid })
...
DynCore.Functions.CreateCallback('dyn-phone:server:GetContactPictures', function(_, cb, Chats)
...
DynCore.Functions.CreateCallback('dyn-phone:server:GetContactPicture', function(_, cb, Chat)
...
DynCore.Functions.CreateCallback('dyn-phone:server:GetPicture', function(_, cb, number)
    local query = '%' .. number .. '%'
    local result = MySQL.query.await('SELECT * FROM players WHERE charinfo LIKE ?', { query })
    if result[1] ~= nil then
...
DynCore.Functions.CreateCallback('dyn-phone:server:FetchResult', function(_, cb, search)
...
DynCore.Functions.CreateCallback('dyn-phone:server:GetVehicleSearchResults', function(_, cb, search)
    search = escape_sqli(search)
    local searchData = {}
    local query = '%' .. search .. '%'
    ...
    if result[1] ~= nil then
        for k, _ in pairs(result) do
            ...
            if player[1] ~= nil then
                ...
                local vehicleInfo = DynCore.Shared.Vehicles[result[k].vehicle]
...
DynCore.Functions.CreateCallback('dyn-phone:server:ScanPlate', function(source, cb, plate)
    local src = source
    local vehicleData
    if plate ~= nil then
        ...
        TriggerClientEvent('DynCore:Notify', src, 'No Vehicle Nearby', 'error')
        cb(nil)
    end
end)

DynCore.Functions.CreateCallback('dyn-phone:server:HasPhone', function(source, cb)
...
DynCore.Functions.CreateCallback('dyn-phone:server:CanTransferMoney', function(source, cb, amount, iban)
    ...
    if (Player.PlayerData.money.bank - amount) >= 0 then
        ...
        local result = MySQL.query.await('SELECT * FROM players WHERE charinfo LIKE ?', { query })
        if result[1] ~= nil then
            local Reciever = DynCore.Functions.GetPlayerByCitizenId(result[1].citizenid)
...
DynCore.Functions.CreateCallback('dyn-phone:server:GetCurrentLawyers', function(_, cb)
    local Lawyers = {}
...
DynCore.Functions.CreateCallback('dyn-phone:server:GetWebhook', function(_, cb)
    if WebHook ~= '' then
        cb(WebHook)
    else
...
RegisterNetEvent('dyn-phone:server:AddAdvert', function(msg, url)
    local src = source
    local Player = DynCore.Functions.GetPlayer(src)
    ...
    TriggerClientEvent('dyn-phone:client:UpdateAdverts', -1, Adverts, '@' .. Player.PlayerData.charinfo.firstname .. '' .. Player.PlayerData.charinfo.lastname)
end)

RegisterNetEvent('dyn-phone:server:DeleteAdvert', function()
    local Player = DynCore.Functions.GetPlayer(source)
    local citizenid = Player.PlayerData.citizenid
    Adverts[citizenid] = nil
    TriggerClientEvent('dyn-phone:client:UpdateAdvertsDel', -1, Adverts)
end)

RegisterNetEvent('dyn-phone:server:SetCallState', function(bool)
    local src = source
    local Ply = DynCore.Functions.GetPlayer(src)
...
RegisterNetEvent('dyn-phone:server:RemoveMail', function(MailId)
    local src = source
    local Player = DynCore.Functions.GetPlayer(src)
...
RegisterNetEvent('dyn-phone:server:sendNewMail', function(mailData)
    local src = source
    local Player = DynCore.Functions.GetPlayer(src)
    if mailData.button == nil then
        MySQL.insert('INSERT INTO player_mails (`citizenid`, `sender`, `subject`, `message`, `mailid`, `read`) VALUES (?, ?, ?, ?, ?, ?)', { Player.PlayerData.citizenid, mailData.sender, mailData.subject, mailData.message, GenerateMailId(), 0 })
...
RegisterNetEvent('dyn-phone:server:sendNewEventMail', function(citizenid, mailData)
    local Player = DynCore.Functions.GetPlayerByCitizenId(citizenid)
    if mailData.button == nil then
        MySQL.insert('INSERT INTO player_mails (`citizenid`, `sender`, `subject`, `message`, `mailid`, `read`) VALUES (?, ?, ?, ?, ?, ?)', { citizenid, mailData.sender, mailData.subject, mailData.message, GenerateMailId(), 0 })
    else
    ...
    SetTimeout(200, function()
        ...
        TriggerClientEvent('dyn-phone:client:UpdateMails', Player.PlayerData.source, mails)
    end)
end)

RegisterNetEvent('dyn-phone:server:ClearButtonData', function(mailId)
    local src = source
    local Player = DynCore.Functions.GetPlayer(src)
...
RegisterNetEvent('dyn-phone:server:MentionedPlayer', function(firstName, lastName, TweetMessage)
    for _, v in pairs(DynCore.Functions.GetPlayers()) do
        local Player = DynCore.Functions.GetPlayer(v)
        if Player ~= nil then
            if (Player.PlayerData.charinfo.firstname == firstName and Player.PlayerData.charinfo.lastname == lastName) then
                ...
                TriggerClientEvent('dyn-phone:client:GetMentioned', Player.PlayerData.source, TweetMessage, AppAlerts[Player.PlayerData.citizenid]['twitter'])
            else
...
RegisterNetEvent('dyn-phone:server:CallContact', function(TargetData, CallId, AnonymousCall)
    local src = source
    local Ply = DynCore.Functions.GetPlayer(src)
    local Target = DynCore.Functions.GetPlayerByPhone(TargetData.number)
    if Target ~= nil then
        TriggerClientEvent('dyn-phone:client:GetCalled', Target.PlayerData.source, Ply.PlayerData.charinfo.phone, CallId, AnonymousCall)
    end
end)

RegisterNetEvent('dyn-phone:server:BillingEmail', function(data, paid)
    for _, v in pairs(DynCore.Functions.GetPlayers()) do
        local target = DynCore.Functions.GetPlayer(v)
        if target.PlayerData.job.name == data.society then
            if paid then
                local name = '' .. DynCore.Functions.GetPlayer(source).PlayerData.charinfo.firstname .. ' ' .. DynCore.Functions.GetPlayer(source).PlayerData.charinfo.lastname .. ''
                TriggerClientEvent('dyn-phone:client:BillingEmail', target.PlayerData.source, data, true, name)
            else
                local name = '' .. DynCore.Functions.GetPlayer(source).PlayerData.charinfo.firstname .. ' ' .. DynCore.Functions.GetPlayer(source).PlayerData.charinfo.lastname .. ''
                TriggerClientEvent('dyn-phone:client:BillingEmail', target.PlayerData.source, data, false, name)
            end
        end
    end
end)

RegisterNetEvent('dyn-phone:server:UpdateHashtags', function(Handle, messageData)
    ...
    TriggerClientEvent('dyn-phone:client:UpdateHashtags', -1, Handle, messageData)
end)

RegisterNetEvent('dyn-phone:server:SetPhoneAlerts', function(app, alerts)
    local src = source
    local CitizenId = DynCore.Functions.GetPlayer(src).citizenid
    QBPhone.SetPhoneAlerts(CitizenId, app, alerts)
end)

RegisterNetEvent('dyn-phone:server:DeleteTweet', function(tweetId)
    local Player = DynCore.Functions.GetPlayer(source)
    local delete = false
    local TID = tweetId
    ...
    if delete then
        ...
        TriggerClientEvent('dyn-phone:client:UpdateTweets', -1, TWData, nil, true)
    end
end)

RegisterNetEvent('dyn-phone:server:UpdateTweets', function(NewTweets, TweetData)
    local src = source
    if Config.Linux then
        ...
        TriggerClientEvent('dyn-phone:client:UpdateTweets', -1, src, NewTweets, TweetData, false)
    else
        MySQL.insert('INSERT INTO phone_tweets (citizenid, firstName, lastName, message, date, url, picture, tweetid) VALUES (?, ?, ?, ?, ?, ?, ?, ?)', {
            TweetData.citizenid,
            TweetData.firstName,
...
RegisterNetEvent('dyn-phone:server:TransferMoney', function(iban, amount)
    local src = source
    local sender = DynCore.Functions.GetPlayer(src)
    ...
    if result[1] ~= nil then
        ...
        if reciever ~= nil then
            ...
            if PhoneItem ~= nil then
                TriggerClientEvent('dyn-phone:client:TransferMoney', reciever.PlayerData.source, amount,
        ...
        TriggerClientEvent('DynCore:Notify', src, "This account number doesn't exist!", 'error')
    end
end)

RegisterNetEvent('dyn-phone:server:EditContact', function(newName, newNumber, newIban, oldName, oldNumber, _)
    local src = source
    local Player = DynCore.Functions.GetPlayer(src)
    MySQL.update(
...
RegisterNetEvent('dyn-phone:server:RemoveContact', function(Name, Number)
    local src = source
    local Player = DynCore.Functions.GetPlayer(src)
...
RegisterNetEvent('dyn-phone:server:AddNewContact', function(name, number, iban)
    local src = source
    local Player = DynCore.Functions.GetPlayer(src)
...
RegisterNetEvent('dyn-phone:server:UpdateMessages', function(ChatMessages, ChatNumber, _)
    local src = source
    local SenderData = DynCore.Functions.GetPlayer(src)
    local query = '%' .. ChatNumber .. '%'
    local Player = MySQL.query.await('SELECT * FROM players WHERE charinfo LIKE ?', { query })
    if Player[1] ~= nil then
        local TargetData = DynCore.Functions.GetPlayerByCitizenId(Player[1].citizenid)
        if TargetData ~= nil then
            ...
            if Chat[1] ~= nil then
                ...
                TriggerClientEvent('dyn-phone:client:UpdateMessages', TargetData.PlayerData.source, ChatMessages, SenderData.PlayerData.charinfo.phone, false)
            else
                -- Insert for target
                ...
                -- Send notification & Update messages for target
                TriggerClientEvent('dyn-phone:client:UpdateMessages', TargetData.PlayerData.source, ChatMessages, SenderData.PlayerData.charinfo.phone, true)
            end
        else
            local Chat = MySQL.query.await('SELECT * FROM phone_messages WHERE citizenid = ? AND number = ?', { SenderData.PlayerData.citizenid, ChatNumber })
            if Chat[1] ~= nil then
...
RegisterNetEvent('dyn-phone:server:AddRecentCall', function(type, data)
    local src = source
    local Ply = DynCore.Functions.GetPlayer(src)
    local Hour = os.date('%H')
    local Minute = os.date('%M')
    local label = Hour .. ':' .. Minute
    TriggerClientEvent('dyn-phone:client:AddRecentCall', src, data, label, type)
    local Trgt = DynCore.Functions.GetPlayerByPhone(data.number)
    if Trgt ~= nil then
        TriggerClientEvent('dyn-phone:client:AddRecentCall', Trgt.PlayerData.source, {
...
RegisterNetEvent('dyn-phone:server:CancelCall', function(ContactData)
    local Ply = DynCore.Functions.GetPlayerByPhone(ContactData.TargetData.number)
    if Ply ~= nil then
        TriggerClientEvent('dyn-phone:client:CancelCall', Ply.PlayerData.source)
    end
end)

RegisterNetEvent('dyn-phone:server:AnswerCall', function(CallData)
    local Ply = DynCore.Functions.GetPlayerByPhone(CallData.TargetData.number)
    if Ply ~= nil then
        TriggerClientEvent('dyn-phone:client:AnswerCall', Ply.PlayerData.source)
    end
end)

RegisterNetEvent('dyn-phone:server:SaveMetaData', function(MData)
    local src = source
    local Player = DynCore.Functions.GetPlayer(src)
...
RegisterNetEvent('dyn-phone:server:GiveContactDetails', function(PlayerId)
    local src = source
    local Player = DynCore.Functions.GetPlayer(src)
    ...
    TriggerClientEvent('dyn-phone:client:AddNewSuggestion', PlayerId, SuggestionData)
end)

RegisterNetEvent('dyn-phone:server:AddTransaction', function(data)
    local src = source
    local Player = DynCore.Functions.GetPlayer(src)
...
RegisterNetEvent('dyn-phone:server:InstallApplication', function(ApplicationData)
    local src = source
    local Player = DynCore.Functions.GetPlayer(src)
    ...
    -- TriggerClientEvent('dyn-phone:RefreshPhone', src)
end)

RegisterNetEvent('dyn-phone:server:RemoveInstallation', function(App)
    local src = source
    local Player = DynCore.Functions.GetPlayer(src)
...
RegisterNetEvent('dyn-phone:server:addImageToGallery', function(image)
    local src = source
    local Player = DynCore.Functions.GetPlayer(src)
    MySQL.insert('INSERT INTO phone_gallery (`citizenid`, `image`) VALUES (?, ?)', { Player.PlayerData.citizenid, image })
end)

RegisterNetEvent('dyn-phone:server:getImageFromGallery', function()
    local src = source
    local Player = DynCore.Functions.GetPlayer(src)
    local images = MySQL.query.await('SELECT * FROM phone_gallery WHERE citizenid = ? ORDER BY `date` DESC', { Player.PlayerData.citizenid })
    TriggerClientEvent('dyn-phone:refreshImages', src, images)
end)

RegisterNetEvent('dyn-phone:server:RemoveImageFromGallery', function(data)
    local src = source
    local Player = DynCore.Functions.GetPlayer(src)
...
RegisterNetEvent('dyn-phone:server:sendPing', function(data)
    local src = source
    if src == data then
        TriggerClientEvent('DynCore:Notify', src, 'You cannot ping yourself', 'error')
    end
end)

-- Command

DynCore.Commands.Add('setmetadata', 'Set Player Metadata (God Only)', {}, false, function(source, args)
    local Player = DynCore.Functions.GetPlayer(source)
    if args[1] then
...
DynCore.Commands.Add('bill', 'Bill A Player', { { name = 'id', help = 'Player ID' }, { name = 'amount', help = 'Fine Amount' } }, false, function(source, args)
    local biller = DynCore.Functions.GetPlayer(source)
    local billed = DynCore.Functions.GetPlayer(tonumber(args[1]))
        ...
        if billed ~= nil then
            if biller.PlayerData.citizenid ~= billed.PlayerData.citizenid then
                if amount and amount > 0 then
                    ...
                    TriggerClientEvent('dyn-phone:RefreshPhone', billed.PlayerData.source)
                    TriggerClientEvent('DynCore:Notify', source, 'Invoice Successfully Sent', 'success')
                    TriggerClientEvent('DynCore:Notify', billed.PlayerData.source, 'New Invoice Received')
                else
                    TriggerClientEvent('DynCore:Notify', source, 'Must Be A Valid Amount Above 0', 'error')
                end
            else
                TriggerClientEvent('DynCore:Notify', source, 'You Cannot Bill Yourself', 'error')
            end
        else
            TriggerClientEvent('DynCore:Notify', source, 'Player Not Online', 'error')
        end
    else
        TriggerClientEvent('DynCore:Notify', source, 'No Access', 'error')
    end
end)
