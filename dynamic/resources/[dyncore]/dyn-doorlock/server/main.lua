...
local function isAuthorized(Player, door, usedLockpick)
	...
	if Config.AdminAccess and DynCore.Functions.HasPermission(Player.PlayerData.source, Config.AdminPermission) then
...
DynCore.Functions.CreateCallback('dyn-doorlock:server:setupDoors', function(_, cb)
	cb(Config.DoorList)
end)

DynCore.Functions.CreateCallback('dyn-doorlock:server:checkItems', function(source, cb, items, needsAll)
...
RegisterNetEvent('dyn-doorlock:server:updateState', function(doorID, locked, src, usedLockpick, unlockAnyway, enableSounds, enableAnimation, sentSource)
	...
	TriggerClientEvent('dyn-doorlock:client:setState', -1, playerId, doorID, locked, src or false, enableSounds, enableAnimation)
	...
	SetTimeout(Config.DoorList[doorID].autoLock, function()
		...
		if Config.DoorStates[doorID] == nil then Config.DoorStates[doorID] = locked elseif Config.DoorStates[doorID] ~= locked then Config.DoorStates[doorID] = nil end
		TriggerClientEvent('dyn-doorlock:client:setState', -1, playerId, doorID, true, src or false, enableSounds, enableAnimation)
	end)
end)

RegisterNetEvent('dyn-doorlock:server:saveNewDoor', function(data, doubleDoor)
	local src = source
	if not DynCore.Functions.HasPermission(src, Config.CommandPermission) and not IsPlayerAceAllowed(src, 'command') then
		if Config.Warnings then
			showWarning(Lang:t("general.warn_no_permission_newdoor", {player = GetPlayerName(src), license = DynCore.Functions.GetIdentifier(src, 'license'), source = src}))
		end
		return
	end
	...
	TriggerClientEvent('dyn-doorlock:client:newDoorAdded', -1, configData, identifier, src)
end)
...
RegisterNetEvent('dyn-doorlock:server:removeLockpick', function(type)
	local Player = DynCore.Functions.GetPlayer(source)
...
DynCore.Commands.Add('newdoor', Lang:t("general.newdoor_command_description"), {}, false, function(source)
	TriggerClientEvent('dyn-doorlock:client:addNewDoor', source)
end, Config.CommandPermission)

DynCore.Commands.Add('doordebug', Lang:t("general.doordebug_command_description"), {}, false, function(source)
	TriggerClientEvent('dyn-doorlock:client:ToggleDoorDebug', source)
end, Config.CommandPermission)
