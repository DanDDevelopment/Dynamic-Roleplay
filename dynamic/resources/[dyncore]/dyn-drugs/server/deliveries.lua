...
-- Callbacks
DynCore.Functions.CreateCallback('dyn-drugs:server:RequestConfig', function(_, cb)
    cb(Config.Dealers)
end)
...
RegisterNetEvent('dyn-drugs:server:updateDealerItems', function(itemData, amount, dealer)
    ...
    if Config.Dealers[dealer]['products'][itemData.slot].amount - 1 >= 0 then
        ...
        TriggerClientEvent('DynCore:Notify', src, Lang:t('error.item_unavailable'), 'error')
    end
end)
...
RegisterNetEvent('dyn-drugs:server:successDelivery', function(deliveryData, inTime)
    ...
    local copsOnline = DynCore.Functions.GetDutyCount('police')
    ...
    if inTime then
        if invItem and invItem.amount >= itemAmount then -- on time correct amount
            ...
            TriggerClientEvent('DynCore:Notify', src, Lang:t('success.order_delivered'), 'success')
            ...
            TriggerClientEvent('DynCore:Notify', src, Lang:t('error.order_not_right'), 'error') -- on time incorrect amount
            if invItem then
        ...
        if invItem and invItem.amount >= itemAmount then -- late correct amount
            TriggerClientEvent('DynCore:Notify', src, Lang:t('error.too_late'), 'error')
            exports['dyn-inventory']:RemoveItem(src, item, itemAmount, false, 'dyn-drugs:server:successDelivery')
...
DynCore.Commands.Add('newdealer', Lang:t('info.newdealer_command_desc'), { {
    ...
    if result then return TriggerClientEvent('DynCore:Notify', source, Lang:t('error.dealer_already_exists'), 'error') end
...
DynCore.Commands.Add('deletedealer', Lang:t('info.deletedealer_command_desc'), { {
    ...
    if result then
        ...
        TriggerClientEvent('dyn-drugs:client:RefreshDealers', -1, Config.Dealers)
        TriggerClientEvent('DynCore:Notify', source, Lang:t('success.dealer_deleted', { dealerName = dealerName }), 'success')
    else
        TriggerClientEvent('DynCore:Notify', source, Lang:t('error.dealer_not_exists_command', { dealerName = dealerName }), 'error')
    end
end, 'admin')

DynCore.Commands.Add('dealers', Lang:t('info.dealers_command_desc'), {}, false, function(source, _)
    local DealersText = ''
    if Config.Dealers ~= nil and next(Config.Dealers) ~= nil then
        ...
        TriggerClientEvent('DynCore:Notify', source, Lang:t('error.no_dealers'), 'error')
    end
end, 'admin')

DynCore.Commands.Add('dealergoto', Lang:t('info.dealergoto_command_desc'), { {
    ...
    if Config.Dealers[DealerName] then
        ...
        TriggerClientEvent('DynCore:Notify', source, Lang:t('success.teleported_to_dealer', { dealerName = DealerName }), 'success')
    else
        TriggerClientEvent('DynCore:Notify', source, Lang:t('error.dealer_not_exists'), 'error')
    end
end, 'admin')
