...
local function TooFarAway()
    DynCore.Functions.Notify(Lang:t('error.too_far_away'), 'error')
...
local function RobberyPed()
    if Config.UseTarget then
        exports['dyn-target']:AddEntityZone('stealingPed', stealingPed, {
            name = 'stealingPed',
            debugPoly = false,
        }, {
            options = {
                {
                    ...
                    action = function()
                        ...
                        TriggerServerEvent('dyn-drugs:server:giveStealItems', stealData.item, stealData.amount)
                        TriggerEvent('dyn-inventory:client:ItemBox', DynCore.Shared.Items[stealData.item], 'add')
                        stealingPed = nil
                        stealData = {}
                        exports['dyn-target']:RemoveZone('stealingPed')
                    end,
                ...
                if IsEntityDead(stealingPed) then
                    ...
                    if not Config.UseTarget and #(pos - pedpos) < 1.5 then
                        if not textDrawn then
                            textDrawn = true
                            exports['dyn-core']:DrawText(Lang:t('info.pick_up_button'))
                        end
...
local function SellToPed(ped)
    ...
    if hasTarget then
        while pedDist < 1.5 and not IsPedDeadOrDying(ped) do
            ...
            if getRobbed <= Config.RobberyChance then
                TriggerServerEvent('dyn-drugs:server:robCornerDrugs', drugType, bagAmount)
                DynCore.Functions.Notify(Lang:t('info.has_been_robbed', { bags = bagAmount, drugType = availableDrugs[drugType].label }))
                stealingPed = ped
                stealData = {
                ...
                if pedDist2 < 1.5 and cornerselling then
                    if Config.UseTarget and not zoneMade then
                        zoneMade = true
                        exports['dyn-target']:AddEntityZone('sellingPed', ped, {
                            name = 'sellingPed',
                            debugPoly = false,
                        }, {
                            options = {
                                {
                                    ...
                                    action = function(entity)
                                        if IsPedInAnyVehicle(PlayerPedId(), false) then
                                            DynCore.Functions.Notify(Lang:t('error.in_vehicle'), 'error')
                                            hasTarget = false
                                            SetPedKeepTask(entity, false)
                                            SetEntityAsNoLongerNeeded(entity)
                                            ClearPedTasksImmediately(entity)
                                            lastPed[#lastPed + 1] = entity
                                            exports['dyn-target']:RemoveZone('sellingPed')
                                            return
                                        else
                                            exports['dyn-target']:RemoveZone('sellingPed')
                                            DynCore.Functions.Progressbar('cornerSelling', Lang:t('info.selling_to_ped'), '5000', false, false, {
                                                disableMovement = true,
                                                disableCarMovement = true,
                                                disableMouse = false,
                                                disableCombat = false,
                                            }, {}, {}, {}, function()
                                                TriggerServerEvent('dyn-drugs:server:sellCornerDrugs', drugType, bagAmount, randomPrice)
                                                hasTarget = false
                                ...
                                },
                                {
                                    ...
                                    label = 'Decline offer',
                                    action = function(entity)
                                        DynCore.Functions.Notify(Lang:t('error.offer_declined'), 'error')
                                        hasTarget = false
                                        SetPedKeepTask(entity, false)
                                        SetEntityAsNoLongerNeeded(entity)
                                        ClearPedTasksImmediately(entity)
                    ...
                    elseif not Config.UseTarget then
                        if not textDrawn then
                            textDrawn = true
                            exports['dyn-core']:DrawText(Lang:t('info.drug_offer', { bags = bagAmount, drugLabel = currentOfferDrug.label, randomPrice = randomPrice }))
                        end
...
local function ToggleSelling()
    if not cornerselling then
        ...
        DynCore.Functions.Notify(Lang:t('info.started_selling_drugs'))
        ...
        CreateThread(function()
            while cornerselling do
                ...
                if not hasTarget then
                    ...
                    local closestPed, closestDistance = DynCore.Functions.GetClosestPed(coords, PlayerPeds)
        ...
        cornerselling = false
        LocalPlayer.state:set('inv_busy', false, true)
        DynCore.Functions.Notify(Lang:t('info.stopped_selling_drugs'))
    end
end

-- Events
RegisterNetEvent('dyn-drugs:client:cornerselling', function()
    DynCore.Functions.TriggerCallback('dyn-drugs:server:cornerselling:getAvailableDrugs', function(result)
        if CurrentCops >= Config.MinimumDrugSalePolice then
            if IsPedInAnyVehicle(PlayerPedId(), false) then
                ...
                if result then
                    ...
                    DynCore.Functions.Notify(Lang:t('error.has_no_drugs'), 'error')
                    LocalPlayer.state:set('inv_busy', false, true)
                end
            end
        else
            DynCore.Functions.Notify(Lang:t('error.not_enough_police', { polices = Config.MinimumDrugSalePolice }), 'error')
        end
    end)
end)
...
RegisterNetEvent('dyn-drugs:client:refreshAvailableDrugs', function(items)
    availableDrugs = items
    if availableDrugs == nil or #availableDrugs <= 0 then
        DynCore.Functions.Notify(Lang:t('error.no_drugs_left'), 'error')
        cornerselling = false
        LocalPlayer.state:set('inv_busy', false, true)
    end
end)
