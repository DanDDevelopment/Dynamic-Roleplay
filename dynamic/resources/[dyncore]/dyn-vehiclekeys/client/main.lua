        ...
        while true do
            ...
            if LocalPlayer.state.isLoggedIn then
                ...
                if entering ~= 0 and not isBlacklistedVehicle(entering) then
                    sleep = 2000
                    local plate = DynCore.Functions.GetPlate(entering)
                    ...
                    if driver ~= 0 and not IsPedAPlayer(driver) and not HasKeys(plate) and not carIsImmune then
                        if IsEntityDead(driver) then
                            if not isTakingKeys then
                                ...
                                TriggerServerEvent('dyn-vehiclekeys:server:setVehLockState', NetworkGetNetworkIdFromEntity(entering), 1)
                                DynCore.Functions.Progressbar('steal_keys', Lang:t('progress.takekeys'), 2500, false, false, {
                                    disableMovement = false,
                                    disableCarMovement = true,
                                    disableMouse = false,
                                    disableCombat = true
                                }, {}, {}, {}, function() -- Done
                                    TriggerServerEvent('dyn-vehiclekeys:server:AcquireVehicleKeys', plate)
                                    isTakingKeys = false
                                end, function()
                                    isTakingKeys = false
                                end)
                        ...
                        elseif Config.LockNPCDrivingCars then
                            TriggerServerEvent('dyn-vehiclekeys:server:setVehLockState', NetworkGetNetworkIdFromEntity(entering), 2)
                        else
                            TriggerServerEvent('dyn-vehiclekeys:server:setVehLockState', NetworkGetNetworkIdFromEntity(entering), 1)
                            TriggerServerEvent('dyn-vehiclekeys:server:AcquireVehicleKeys', plate)
                    ...
                    elseif driver == 0 and entering ~= lastPickedVehicle and not HasKeys(plate) and not isTakingKeys then
                        DynCore.Functions.TriggerCallback('dyn-vehiclekeys:server:checkPlayerOwned', function(playerOwned)
...
AddEventHandler('onResourceStart', function(resourceName)
    if resourceName == GetCurrentResourceName() and DynCore.Functions.GetPlayerData() ~= {} then
        GetKeys()
    end
end)
...
RegisterNetEvent('dyn-vehiclekeys:client:AddKeys', function(plate)
    KeysList[plate] = true
    local ped = PlayerPedId()
    if IsPedInAnyVehicle(ped, false) then
        local vehicle = GetVehiclePedIsIn(ped)
        local vehicleplate = DynCore.Functions.GetPlate(vehicle)
...
RegisterNetEvent('dyn-vehiclekeys:client:RemoveKeys', function(plate)
    KeysList[plate] = nil
end)

RegisterNetEvent('dyn-vehiclekeys:client:ToggleEngine', function()
    local EngineOn = GetIsVehicleEngineRunning(GetVehiclePedIsIn(PlayerPedId()))
    local vehicle = GetVehiclePedIsIn(PlayerPedId(), true)
    if HasKeys(DynCore.Functions.GetPlate(vehicle)) then
        if EngineOn then
            SetVehicleEngineOn(vehicle, false, false, true)
        else
...
RegisterNetEvent('dyn-vehiclekeys:client:GiveKeys', function(id)
    local targetVehicle = GetVehicle()
    if targetVehicle then
        local targetPlate = DynCore.Functions.GetPlate(targetVehicle)
            ...
            if id and type(id) == 'number' then -- Give keys to specific ID
                ...
                if IsPedSittingInVehicle(PlayerPedId(), targetVehicle) then -- Give keys to everyone in vehicle
                    ...
                    for p = 1, #otherOccupants do
                        TriggerServerEvent('dyn-vehiclekeys:server:GiveVehicleKeys', GetPlayerServerId(NetworkGetPlayerIndexFromPed(otherOccupants[p])), targetPlate)
                    end
                else -- Give keys to closest player
                    GiveKeys(GetPlayerServerId(DynCore.Functions.GetClosestPlayer()), targetPlate)
                end
            end
        else
            DynCore.Functions.Notify(Lang:t('notify.ydhk'), 'error')
        end
    end
end)

RegisterNetEvent('DynCore:Client:EnteringVehicle', function()
    robKeyLoop()
end)

RegisterNetEvent('dyn-weapons:client:DrawWeapon', function()
    Wait(2000)
    robKeyLoop()
end)
...
RegisterNetEvent('lockpicks:UseLockpick', function(isAdvanced)
    ...
    if HasKeys(DynCore.Functions.GetPlate(vehicle)) then return end
    if #(pos - GetEntityCoords(vehicle)) > 2.5 then return end
    if GetVehicleDoorLockStatus(vehicle) <= 0 then return end

    local difficulty = isAdvanced and 'easy' or 'medium' -- Easy for advanced lockpick, medium by default
    local success = exports['dyn-minigames']:Skillbar(difficulty)
    ...
    if success then
        ...
        if GetPedInVehicleSeat(vehicle, -1) == PlayerPedId() then
            TriggerServerEvent('dyn-vehiclekeys:server:AcquireVehicleKeys', DynCore.Functions.GetPlate(vehicle))
        else
            DynCore.Functions.Notify(Lang:t('notify.vlockpick'), 'success')
            TriggerServerEvent('dyn-vehiclekeys:server:setVehLockState', NetworkGetNetworkIdFromEntity(vehicle), 1)
        end
    else
        TriggerServerEvent('hud:server:GainStress', math.random(1, 4))
    ...
    if isAdvanced then
        if chance <= Config.RemoveLockpickAdvanced then
            TriggerServerEvent('dyn-vehiclekeys:server:breakLockpick', 'advancedlockpick')
        end
    else
        if chance <= Config.RemoveLockpickNormal then
            TriggerServerEvent('dyn-vehiclekeys:server:breakLockpick', 'lockpick')
        end
    end
end)
...
function ToggleEngine(veh)
    if veh then
        ...
        if not isBlacklistedVehicle(veh) then
            if HasKeys(DynCore.Functions.GetPlate(veh)) or AreKeysJobShared(veh) then
                if EngineOn then
        ...
        if not isBlacklistedVehicle(veh) then
            if HasKeys(DynCore.Functions.GetPlate(veh)) or AreKeysJobShared(veh) then
                ...
                if vehLockStatus == 1 then
                    TriggerServerEvent('dyn-vehiclekeys:server:setVehLockState', NetworkGetNetworkIdFromEntity(veh), 2)
                    DynCore.Functions.Notify(Lang:t('notify.vlock'), 'primary')
                else
                    TriggerServerEvent('dyn-vehiclekeys:server:setVehLockState', NetworkGetNetworkIdFromEntity(veh), 1)
                    DynCore.Functions.Notify(Lang:t('notify.vunlock'), 'success')
                end
...
function GiveKeys(id, plate)
    ...
    if distance < 1.5 and distance > 0.0 then
        TriggerServerEvent('dyn-vehiclekeys:server:GiveVehicleKeys', id, plate)
    else
        DynCore.Functions.Notify(Lang:t('notify.nonear'), 'error')
    end
end

function GetKeys()
    DynCore.Functions.TriggerCallback('dyn-vehiclekeys:server:GetVehicleKeys', function(keysList)
        KeysList = keysList
    end)
end
...
function GetVehicle()
    ...
    while vehicle == 0 do
        vehicle = DynCore.Functions.GetClosestVehicle()
...
function AreKeysJobShared(veh)
    ...
    local vehPlate = DynCore.Functions.GetPlate(veh)
    local jobName = DynCore.Functions.GetPlayerData().job.name
    local onDuty = DynCore.Functions.GetPlayerData().job.onduty
            ...
            for _, vehicle in pairs(v.vehicles) do
                if string.upper(vehicle) == string.upper(vehName) then
                    if not HasKeys(vehPlate) then
                        TriggerServerEvent('dyn-vehiclekeys:server:AcquireVehicleKeys', vehPlate)
                    end
                    return true
                end
            end
...
function Hotwire(vehicle, plate)
    ...
    DynCore.Functions.Progressbar('hotwire_vehicle', Lang:t('progress.hskeys'), hotwireTime, false, true, {
        ...
        if (math.random() <= Config.HotwireChance) then
            ...
            DynCore.Functions.Notify(Lang:t('notify.fvlockpick'), 'error')
        end
...
function CarjackVehicle(target)
    ...
    DynCore.Functions.Progressbar('rob_keys', Lang:t('progress.acjack'), Config.CarjackingTime, false, true, {}, {}, {}, {}, function()
        ...
        if hasWeapon and isCarjacking then
            ...
            if math.random() <= carjackChance then
                ...
                DynCore.Functions.Notify(Lang:t('notify.cjackfail'), 'error')
                FreezeEntityPosition(vehicle, false)
                SetVehicleUndriveable(vehicle, false)
                MakePedFlee(target)
                TriggerServerEvent('hud:server:GainStress', math.random(1, 4))
