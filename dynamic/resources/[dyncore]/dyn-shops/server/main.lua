...
local function deliveryPay(source, shop)
    ...
    Player.Functions.AddMoney('bank', Config.DeliveryPrice, 'dyn-shops:deliveryPay')
    if math.random(100) <= 10 then exports['dyn-inventory']:AddItem(source, Config.RewardItem, 1, false, false, 'dyn-shops:deliveryPay') end
end
...
RegisterNetEvent('dyn-shops:server:RestockShopItems', function(shop)
    local src = source
    ...
    TriggerClientEvent('dyn-shops:client:SetShopItems', -1, shop, Config.Locations[shop].products)
end)

RegisterNetEvent('dyn-shops:server:UpdateShopItems', function(shop, itemData, amount) -- called from inventory
...
RegisterNetEvent('dyn-shops:server:DoBail', function(bool)
    local src = source
    ...
    if bool then
        if Player.Functions.RemoveMoney('cash', Config.TruckDeposit, 'tow-received-bail') then
            ...
            TriggerClientEvent('DynCore:Notify', src, Lang:t('success.paid_with_cash', { value = Config.TruckDeposit }), 'success')
            TriggerClientEvent('dyn-shops:client:SpawnVehicle', src)
        elseif Player.Functions.RemoveMoney('bank', Config.TruckDeposit, 'tow-received-bail') then
            Bail[Player.PlayerData.citizenid] = Config.TruckDeposit
            TriggerClientEvent('DynCore:Notify', src, Lang:t('success.paid_with_bank', { value = Config.TruckDeposit }), 'success')
            TriggerClientEvent('dyn-shops:client:SpawnVehicle', src)
        else
            TriggerClientEvent('DynCore:Notify', src, Lang:t('error.no_deposit', { value = Config.TruckDeposit }), 'error')
        end
    else
        if Bail[Player.PlayerData.citizenid] then
            Player.Functions.AddMoney('cash', Bail[Player.PlayerData.citizenid], 'trucker-bail-paid')
            Bail[Player.PlayerData.citizenid] = nil
            TriggerClientEvent('DynCore:Notify', src, Lang:t('success.refund_to_cash', { value = Config.TruckDeposit }), 'success')
        end
    end
end)

RegisterNetEvent('dyn-shops:server:PaySlip', function(drops)
    local src = source
    ...
    TriggerClientEvent('DynCore:Notify', src, Lang:t('success.you_earned', { value = payment }), 'success')
end)

-- Opening shops

RegisterNetEvent('dyn-shops:server:openShop', function(data)
    local src = source
    ...
    exports['dyn-inventory']:OpenShop(src, shopName)
end)
