...
local function OpenGarageMenu(data)
    QBCore.Functions.TriggerCallback('dyn-garages:server:GetGarageVehicles', function(result)
...
local function DepositVehicle(veh, data)
    ...
    QBCore.Functions.TriggerCallback('dyn-garages:server:canDeposit', function(canDeposit)
        if canDeposit then
            local bodyDamage = math.ceil(GetVehicleBodyHealth(veh))
            local engineDamage = math.ceil(GetVehicleEngineHealth(veh))
            local totalFuel = exports[Config.FuelResource]:GetFuel(veh)
            TriggerServerEvent('dyn-mechanicjob:server:SaveVehicleProps', QBCore.Functions.GetVehicleProperties(veh))
...
RegisterNetEvent('dyn-garages:client:takeOutGarage', function(data)
    QBCore.Functions.TriggerCallback('dyn-garages:server:IsSpawnOk', function(spawn)
        if spawn then
            local location = GetSpawnPoint(data.garage)
            if not location then return end
            QBCore.Functions.TriggerCallback('dyn-garages:server:spawnvehicle', function(netId, properties, vehPlate)
...
RegisterNetEvent('dyn-garages:client:setHouseGarage', function(house, hasKey) -- event sent periodically from housing
    ...
    if Config.Garages[formattedHouseName] then
        ...
        QBCore.Functions.TriggerCallback('dyn-garages:server:getHouseGarage', function(garageInfo) -- create garage if not exist
