...
RegisterNetEvent('dyn-vehiclekeys:server:breakLockpick', function(itemName)
    ...
    if exports['dyn-inventory']:RemoveItem(source, itemName, 1, false, 'dyn-vehiclekeys:server:breakLockpick') then
        TriggerClientEvent('dyn-inventory:client:ItemBox', source, QBCore.Shared.Items[itemName], 'remove')
    end
end)
...
QBCore.Functions.CreateCallback('dyn-vehiclekeys:server:GetVehicleKeys', function(source, cb)
    local Player = QBCore.Functions.GetPlayer(source)
    if not Player then return end
...
QBCore.Functions.CreateCallback('dyn-vehiclekeys:server:checkPlayerOwned', function(_, cb, plate)
