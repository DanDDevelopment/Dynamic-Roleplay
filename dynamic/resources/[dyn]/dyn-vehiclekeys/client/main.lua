            ...
            if LocalPlayer.state.isLoggedIn then
                ...
                if entering ~= 0 and not isBlacklistedVehicle(entering) then
                    ...
                    elseif driver == 0 and entering ~= lastPickedVehicle and not HasKeys(plate) and not isTakingKeys then
                        QBCore.Functions.TriggerCallback('dyn-vehiclekeys:server:checkPlayerOwned', function(playerOwned)
...
RegisterNetEvent('lockpicks:UseLockpick', function(isAdvanced)
    ...
    if success then
        ...
        if GetPedInVehicleSeat(vehicle, -1) == PlayerPedId() then
            TriggerServerEvent('dyn-vehiclekeys:server:AcquireVehicleKeys', QBCore.Functions.GetPlate(vehicle))
        else
...
function GetKeys()
    QBCore.Functions.TriggerCallback('dyn-vehiclekeys:server:GetVehicleKeys', function(keysList)
        KeysList = keysList
    end)
end
