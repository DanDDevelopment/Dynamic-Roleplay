...
local function isAuthorized(door)
	...
	if door.items then
		local p = promise.new()
		QBCore.Functions.TriggerCallback('dyn-doorlock:server:checkItems', function(result)
			p:resolve(result)
...
function SetupDoors()
	local p = promise.new()
	QBCore.Functions.TriggerCallback('dyn-doorlock:server:setupDoors', function(result)
		p:resolve(result)
	end)
