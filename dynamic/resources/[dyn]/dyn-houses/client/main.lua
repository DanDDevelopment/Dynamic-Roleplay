...
local function SetClosestHouse()
    ...
    if not IsInside then
        ...
        if ClosestHouse ~= nil and tonumber(dist) < 30 then
            QBCore.Functions.TriggerCallback('dyn-houses:server:ProximityKO', function(key, owned)
                HasHouseKey = key
                isOwned = owned
            end, ClosestHouse)
        end
...
local function setHouseLocations()
    if ClosestHouse ~= nil then
        QBCore.Functions.TriggerCallback('dyn-houses:server:getHouseLocations', function(result)
            if result ~= nil then
...
local function LoadDecorations(house)
    if Config.Houses[house].decorations == nil or next(Config.Houses[house].decorations) == nil then
        QBCore.Functions.TriggerCallback('dyn-houses:server:getHouseDecorations', function(result)
...
local function getKeyHolders()
    ...
    local p = promise.new()
    QBCore.Functions.TriggerCallback('dyn-houses:server:getHouseKeyHolders', function(holders)
        p:resolve(holders)
    end, ClosestHouse)
...
RegisterNetEvent('dyn-houses:client:removeHouseKey', function()
    if ClosestHouse ~= nil then
        ...
        local housedist = #(pedpos - vector3(Config.Houses[ClosestHouse].coords.enter.x, Config.Houses[ClosestHouse].coords.enter.y, Config.Houses[ClosestHouse].coords.enter.z))
        if housedist <= 5 then
            QBCore.Functions.TriggerCallback('dyn-houses:server:getHouseOwner', function(result)
...
RegisterNetEvent('dyn-houses:client:setupHouseBlips', function() -- Setup owned on load
    CreateThread(function()
        ...
        if LocalPlayer.state['isLoggedIn'] then
            QBCore.Functions.TriggerCallback('dyn-houses:server:getOwnedHouses', function(ownedHouses)
                if ownedHouses then
...
RegisterNUICallback('HasEnoughMoney', function(cData, cb)
    QBCore.Functions.TriggerCallback('dyn-houses:server:HasEnoughMoney', function(_)
        cb('ok')
