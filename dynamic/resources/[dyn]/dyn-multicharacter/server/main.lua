...
RegisterNetEvent('dyn-multicharacter:server:loadUserData', function(cData)
    ...
    if QBCore.Player.Login(src, cData.citizenid) then
        ...
        TriggerEvent("dyn-log:server:CreateLog", "joinleave", "Loaded", "green", "**".. GetPlayerName(src) .. "** (<@"..(QBCore.Functions.GetIdentifier(src, 'discord'):gsub("discord:", "") or "unknown").."> |  ||"  ..(QBCore.Functions.GetIdentifier(src, 'ip') or 'undefined') ..  "|| | " ..(QBCore.Functions.GetIdentifier(src, 'license') or 'undefined') .." | " ..cData.citizenid.." | "..src..") loaded..")
    end
end)
...
QBCore.Functions.CreateCallback("dyn-multicharacter:server:GetUserCharacters", function(source, cb)
    local src = source
...
QBCore.Functions.CreateCallback("dyn-multicharacter:server:GetServerLogs", function(_, cb)
    MySQL.query('SELECT * FROM server_logs', {}, function(result)
        cb(result)
    end)
end)

QBCore.Functions.CreateCallback("dyn-multicharacter:server:GetNumberOfCharacters", function(source, cb)
    local src = source
    local license = QBCore.Functions.GetIdentifier(src, 'license')
    local numOfChars = 0
...
QBCore.Functions.CreateCallback("dyn-multicharacter:server:setupCharacters", function(source, cb)
...
QBCore.Functions.CreateCallback("dyn-multicharacter:server:getSkin", function(_, cb, cid)
