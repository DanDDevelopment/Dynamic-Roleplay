...
QBCore.Functions.CreateCallback('dyn-weapons:server:GetConfig', function(_, cb)
...
QBCore.Functions.CreateCallback('dyn-weapons:server:RepairWeapon', function(source, cb, RepairPoint, data)
    local src = source
    ...
    TriggerClientEvent('dyn-inventory:client:ItemBox', src, QBCore.Shared.Items[data.name], 'remove')
...
RegisterNetEvent('dyn-weapons:server:TakeBackWeapon', function(k)
    ...
    TriggerClientEvent('dyn-inventory:client:ItemBox', src, QBCore.Shared.Items[itemdata.name], 'add')
...
local function EquipWeaponTint(source, tintIndex, item, isMK2)
    ...
    TriggerClientEvent('dyn-inventory:client:ItemBox', source, QBCore.Shared.Items[item], 'remove')
...
QBCore.Functions.CreateCallback('dyn-weapons:server:RemoveAttachment', function(source, cb, AttachmentData, WeaponData)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    ...
    if Inventory[WeaponData.slot] then
        if Inventory[WeaponData.slot].info.attachments and next(Inventory[WeaponData.slot].info.attachments) then
            ...
            if HasAttach then
                ...
                TriggerClientEvent('dyn-inventory:client:ItemBox', src, QBCore.Shared.Items[AttachmentData.attachment], 'add')
