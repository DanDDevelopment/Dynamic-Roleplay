...
AddEventHandler('QBCore:Client:OnPlayerLoaded', function()
    ...
    QBCore.Functions.TriggerCallback('dyn-weapons:server:GetConfig', function(RepairPoints)
...
RegisterNetEvent('dyn-weapons:client:AddAmmo', function(type, amount, itemData)
    ...
    QBCore.Functions.Progressbar('taking_bullets', Lang:t('info.loading_bullets'), Config.ReloadTime, false, true, {
        ...
        if QBCore.Shared.Weapons[weapon] then
            ...
            TriggerEvent('dyn-inventory:client:ItemBox', QBCore.Shared.Items[itemData.name], 'remove')
                ...
                if IsPedShooting(ped) or IsControlJustPressed(0, 24) then
                    ...
                    if CanShoot then
                        ...
                        if weapon ~= `WEAPON_UNARMED` then
                            TriggerEvent('dyn-weapons:client:CheckWeapon', QBCore.Shared.Weapons[weapon]['name'])
                            ...
                            if CurrentWeaponData and next(CurrentWeaponData) then
                                if not data.RepairingData.Ready then
                                    ...
                                    if IsControlJustPressed(0, 38) then
                                        QBCore.Functions.TriggerCallback('dyn-weapons:server:RepairWeapon', function(HasMoney)
                                            if HasMoney then
                                                CurrentWeaponData = {}
                                            end
