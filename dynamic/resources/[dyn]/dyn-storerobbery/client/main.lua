                            ...
                            if IsControlJustPressed(0, 38) then
                                if CurrentCops >= Config.MinimumStoreRobberyPolice then
                                    ...
                                    if Config.Safes[safe].type == 'keypad' then
                                        ...
                                        QBCore.Functions.TriggerCallback('dyn-storerobbery:server:getPadlockCombination', function(combination)
                                    ...
                                    if not copsCalled then
                                        ...
                                        TriggerServerEvent('dyn-storerobbery:server:callCops', 'safe', currentSafe, streetLabel, pos)
                                        copsCalled = true
                                    end
                                else
            ...
            if CurrentCops >= Config.MinimumStoreRobberyPolice then
                if usingAdvanced then
                    ...
                    if not copsCalled then
                        ...
                        TriggerServerEvent('dyn-storerobbery:server:callCops', 'cashier', currentRegister, streetLabel, pos)
                        copsCalled = true
                    end
                else
                    lockpick(true)
...
function setupRegister()
    QBCore.Functions.TriggerCallback('dyn-storerobbery:server:getRegisterStatus', function(Registers)
...
function setupSafes()
    QBCore.Functions.TriggerCallback('dyn-storerobbery:server:getSafeStatus', function(Safes)
...
RegisterNUICallback('success', function(_, cb)
    if currentRegister ~= 0 then
        lockpick(false)
        TriggerServerEvent('dyn-storerobbery:server:setRegisterStatus', currentRegister)
        ...
        QBCore.Functions.Progressbar('search_register', Lang:t('text.emptying_the_register'), lockpickTime, false, true, {
            ...
            TriggerServerEvent('dyn-storerobbery:server:takeMoney', currentRegister, true)
        end, function() -- Cancel
            openingDoor = false
            ClearPedTasks(PlayerPedId())
...
function LockpickDoorAnim(time)
    ...
    CreateThread(function()
        while openingDoor do
            ...
            Wait(2000)
            time = time - 2
            TriggerServerEvent('dyn-storerobbery:server:takeMoney', currentRegister, false)
            if time <= 0 then
                openingDoor = false
        ...
        if won then
            if currentSafe ~= 0 then
                if not Config.Safes[currentSafe].robbed then
                    ...
                    TriggerServerEvent('dyn-storerobbery:server:SafeReward', currentSafe)
                    TriggerServerEvent('dyn-storerobbery:server:setSafeStatus', currentSafe)
                    currentSafe = 0
                    takeAnim()
                end
            else
...
RegisterNUICallback('fail', function(_, cb)
    if usingAdvanced then
        if math.random(1, 100) < 20 then
            TriggerServerEvent('dyn-storerobbery:server:removeAdvancedLockpick')
            TriggerEvent('dyn-inventory:client:ItemBox', QBCore.Shared.Items['advancedlockpick'], 'remove')
        end
    else
        if math.random(1, 100) < 40 then
            TriggerServerEvent('dyn-storerobbery:server:removeLockpick')
            TriggerEvent('dyn-inventory:client:ItemBox', QBCore.Shared.Items['lockpick'], 'remove')
        end
    end
...
RegisterNUICallback('TryCombination', function(data, cb)
    QBCore.Functions.TriggerCallback('dyn-storerobbery:server:isCombinationRight', function(combination)
...
RegisterNetEvent('dyn-storerobbery:client:setRegisterStatus', function(batch, val)
...
RegisterNetEvent('dyn-storerobbery:client:setSafeStatus', function(safe, bool)
    Config.Safes[safe].robbed = bool
end)

RegisterNetEvent('dyn-storerobbery:client:robberyCall', function(_, _, _, coords)
