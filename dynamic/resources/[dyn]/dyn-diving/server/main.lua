...
RegisterNetEvent('dyn-diving:server:CallCops', function(coords)
    for _, Player in pairs(QBCore.Functions.GetQBPlayers()) do
        if Player then
            if Player.PlayerData.job.type == 'leo' and Player.PlayerData.job.onduty then
                local msg = Lang:t('info.cop_msg')
                TriggerClientEvent('dyn-diving:client:CallCops', Player.PlayerData.source, coords, msg)
                local alertData = {
...
RegisterNetEvent('dyn-diving:server:SellCoral', function()
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    ...
    if hasCoral(src) then
        for _, v in pairs(availableCoral) do
            ...
            exports['dyn-inventory']:RemoveItem(src, item.name, item.amount, false, 'dyn-diving:server:SellCoral')
            Player.Functions.AddMoney('cash', reward, 'dyn-diving:server:SellCoral')
            TriggerClientEvent('dyn-inventory:client:ItemBox', src, QBCore.Shared.Items[item.name], 'remove')
        end
    else
...
RegisterNetEvent('dyn-diving:server:TakeCoral', function(area, coral, bool)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if not Player then return end
    ...
    if amount > 1 then
        for _ = 1, amount, 1 do
            exports['dyn-inventory']:AddItem(src, ItemData['name'], 1, false, false, 'dyn-diving:server:TakeCoral')
            TriggerClientEvent('dyn-inventory:client:ItemBox', src, ItemData, 'add')
            Wait(250)
        end
    else
        exports['dyn-inventory']:AddItem(src, ItemData['name'], amount, false, false, 'dyn-diving:server:TakeCoral')
        TriggerClientEvent('dyn-inventory:client:ItemBox', src, ItemData, 'add')
    end
    if (Config.CoralLocations[area].TotalCoral - 1) == 0 then
        ...
        TriggerClientEvent('dyn-diving:client:NewLocations', -1)
    else
    ...
    TriggerClientEvent('dyn-diving:client:UpdateCoral', -1, area, coral, bool)
end)

RegisterNetEvent('dyn-diving:server:removeItemAfterFill', function()
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    exports['dyn-inventory']:RemoveItem(src, 'diving_fill', 1, false, 'dyn-diving:server:removeItemAfterFill')
    TriggerClientEvent('dyn-inventory:client:ItemBox', src, QBCore.Shared.Items['diving_fill'], 'remove')
end)

-- Callbacks

QBCore.Functions.CreateCallback('dyn-diving:server:GetDivingConfig', function(_, cb)
...
QBCore.Functions.CreateUseableItem('diving_gear', function(source)
    TriggerClientEvent('dyn-diving:client:UseGear', source)
end)

QBCore.Functions.CreateUseableItem('diving_fill', function(source)
    TriggerClientEvent('dyn-diving:client:setoxygenlevel', source)
end)
