...
RegisterNetEvent('QBCore:Client:UpdateObject', function() QBCore = exports['dyn-core']:GetCoreObject() end)
		...
		if v["icon"] then
			if QBCore.Shared.Items[tostring(v["icon"])] then
				if not string.find(QBCore.Shared.Items[tostring(v["icon"])].image, "//") and not string.find(v["icon"], "//") then
                    v["icon"] = "nui://dyn-inventory/html/images/"..QBCore.Shared.Items[tostring(v["icon"])].image
				end
			end
		end
	end
