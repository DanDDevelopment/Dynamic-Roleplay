...
QBCore.Functions.CreateCallback('dyn-phone:server:GetCallState', function(_, cb, ContactData)
...
QBCore.Functions.CreateCallback('dyn-phone:server:GetPhoneData', function(source, cb)
    local src = source
...
QBCore.Functions.CreateCallback('dyn-phone:server:PayInvoice', function(source, cb, society, amount, invoiceId, sendercitizenid)
    local Invoices = {}
...
QBCore.Functions.CreateCallback('dyn-phone:server:DeclineInvoice', function(source, cb, _, _, invoiceId)
    local Invoices = {}
    local Ply = QBCore.Functions.GetPlayer(source)
    MySQL.query('DELETE FROM phone_invoices WHERE id = ?', { invoiceId })
    local invoices = MySQL.query.await('SELECT * FROM phone_invoices WHERE citizenid = ?', { Ply.PlayerData.citizenid })
...
QBCore.Functions.CreateCallback('dyn-phone:server:GetContactPictures', function(_, cb, Chats)
...
QBCore.Functions.CreateCallback('dyn-phone:server:GetContactPicture', function(_, cb, Chat)
...
QBCore.Functions.CreateCallback('dyn-phone:server:GetPicture', function(_, cb, number)
    local query = '%' .. number .. '%'
    local result = MySQL.query.await('SELECT * FROM players WHERE charinfo LIKE ?', { query })
    if result[1] ~= nil then
...
QBCore.Functions.CreateCallback('dyn-phone:server:FetchResult', function(_, cb, search)
...
QBCore.Functions.CreateCallback('dyn-phone:server:GetVehicleSearchResults', function(_, cb, search)
    search = escape_sqli(search)
    local searchData = {}
    local query = '%' .. search .. '%'
...
QBCore.Functions.CreateCallback('dyn-phone:server:ScanPlate', function(source, cb, plate)
    local src = source
    local vehicleData
    if plate ~= nil then
...
QBCore.Functions.CreateCallback('dyn-phone:server:HasPhone', function(source, cb)
...
QBCore.Functions.CreateCallback('dyn-phone:server:CanTransferMoney', function(source, cb, amount, iban)
...
QBCore.Functions.CreateCallback('dyn-phone:server:GetCurrentLawyers', function(_, cb)
    local Lawyers = {}
...
QBCore.Functions.CreateCallback('dyn-phone:server:GetWebhook', function(_, cb)
    if WebHook ~= '' then
        cb(WebHook)
    else
