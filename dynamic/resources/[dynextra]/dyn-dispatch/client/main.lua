...
local function openMenu()
    ...
    local calls = lib.callback.await('dyn-dispatch:callback:getCalls', false)
    if #calls == 0 then
...
local function setWaypoint()
    ...
    local data = lib.callback.await('dyn-dispatch:callback:getLatestDispatch', false)
    ...
    if not waypointCooldown and lib.table.contains(data.jobs, PlayerData.job.type) then
        ...
        TriggerServerEvent('dyn-dispatch:server:attach', data.id, PlayerData)
...
-- Events
RegisterNetEvent('dyn-dispatch:client:notify', function(data, source)
    if data.alertTime == nil then data.alertTime = Config.AlertTime end
    local timer = data.alertTime * 1000
...
RegisterNetEvent('dyn-dispatch:client:openMenu', function(data)
    if not isJobValid(PlayerData.job.type) then return end
...
RegisterNUICallback("detachUnit", function(data, cb)
    TriggerServerEvent('dyn-dispatch:server:detach', data.id, PlayerData)
    DeleteWaypoint()
    cb("ok")
end)
...
RegisterNUICallback("refreshAlerts", function(data, cb)
    ...
    local data = lib.callback.await('dyn-dispatch:callback:getCalls', false)
    SendNUIMessage({ action = 'setDispatchs', data = data, })
    cb("ok")
end)
