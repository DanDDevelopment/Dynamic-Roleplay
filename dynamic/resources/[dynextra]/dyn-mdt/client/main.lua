...
CreateThread(function()
    if GetResourceState('dyn-dispatch') == 'started' then
        TriggerServerEvent("dyn-mdt:dispatchStatus", true)
    end
end)
...
RegisterNetEvent('DynCore:Client:OnPlayerUnload', function()
    TriggerServerEvent("dyn-mdt:server:OnPlayerUnload")
    PlayerData = {}
end)
...
RegisterNetEvent("DynCore:Client:SetDuty", function(job, state)
    if AllowedJob(job) then
        TriggerServerEvent("dyn-mdt:server:ToggleDuty")
	TriggerServerEvent("dyn-mdt:server:ClockSystem")
...
-- Uses the dyn-Core bill command to send a fine to a player
...
RegisterNUICallback("sendToCommunityService", function(data, cb)
    ...
    if sentence > 0 then
        TriggerServerEvent("dyn-communityservice:server:StartCommunityService", targetSourceId, sentence)
    end
end)
...
RegisterNUICallback("removeCallBlip", function(data, cb)
    TriggerEvent('dyn-dispatch:client:removeCallBlip', data.callid or data.id)
    cb(true)
end)
...
RegisterNetEvent('dyn-mdt:client:selfregister')
AddEventHandler('dyn-mdt:client:selfregister', function()
...
if Config.UseWolfknightRadar == true then
    RegisterNetEvent("dyn-mdt:client:trafficStop")
    AddEventHandler("dyn-mdt:client:trafficStop", function()
