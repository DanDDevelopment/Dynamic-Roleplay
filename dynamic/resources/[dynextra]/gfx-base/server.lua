...
Utils = {
    ["qb"] = {
        ...
        ["GetPlayer"] = function(src)
            return DynCore.Functions.GetPlayer(src)
        end,
        ["GetPlayerIndex"] = function(index)
            return DynCore.Functions.GetPlayerByCitizenId(index)
        end,
        ...
        ["Notify"] = function(src, text)
            return DynCore.Functions.Notify(src, text)
        end,
        ["CreateCallback"] = function(name, func)
            return DynCore.Functions.CreateCallback(name, func)
        end,
        ...
        ["GetItem"] = function(player, item)
            ...
            if GFXInventory then
                ...
                itemData = exports["dyn-inventory"]:GetItemByName(player.PlayerData.source, item)
            end
            return itemData
        end,
        ...
        ["RandomInt"] = function(number)
            return DynCore.Shared.RandomInt(number)
        end,
        ["RandomStr"] = function(number)
            return DynCore.Shared.RandomStr(number)
        end,
        ["IsMod"] = function(player)
            return DynCore.Functions.HasPermission(player.PlayerData.source, "mod")
        end,
        ["IsAdmin"] = function(player)
            return DynCore.Functions.HasPermission(player.PlayerData.source, {"admin","god"})
        end,
    },
    ["esx"] = {
        ["Framework"] = Framework,
