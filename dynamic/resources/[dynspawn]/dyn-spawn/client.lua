...
RegisterNetEvent('dyn-spawn:client:openUI', function(value)
...
RegisterNetEvent('dyn-spawn:client:setupSpawns', function(cData, new, apps)
    if not new then
        DynCore.Functions.TriggerCallback('dyn-spawn:server:getOwnedHouses', function(houses)
            local myHouses = {}
            if houses ~= nil then
                for i = 1, (#houses), 1 do
                    myHouses[#myHouses+1] = {
...
RegisterNUICallback('spawnplayer', function(data, cb)
    ...
    if type == "current" then
        PreSpawnPlayer()
        DynCore.Functions.GetPlayerData(function(pd)
            ped = PlayerPedId()
        ...
        if insideMeta.house ~= nil then
            ...
            TriggerEvent('dyn-houses:client:LastLocationHouse', houseId)
        elseif insideMeta.apartment.apartmentType ~= nil or insideMeta.apartment.apartmentId ~= nil then
            local apartmentType = insideMeta.apartment.apartmentType
            local apartmentId = insideMeta.apartment.apartmentId
            TriggerEvent('dyn-apartments:client:LastLocationHouse', apartmentType, apartmentId)
        end
        TriggerServerEvent('DynCore:Server:OnPlayerLoaded')
        TriggerEvent('DynCore:Client:OnPlayerLoaded')
        PostSpawnPlayer()
    elseif type == "house" then
        PreSpawnPlayer()
        TriggerEvent('dyn-houses:client:enterOwnedHouse', location)
        TriggerServerEvent('DynCore:Server:OnPlayerLoaded')
        TriggerEvent('DynCore:Client:OnPlayerLoaded')
