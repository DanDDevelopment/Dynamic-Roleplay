...
RegisterNetEvent('DynCore:Client:UpdateObject', function() DynCore = exports['dyn-core']:GetCoreObject() end)
...
local function openMenu(data, sort, skipFirst)
	...
	for _,v in pairs(data) do
		if v["icon"] then
			if DynCore.Shared.Items[tostring(v["icon"])] then
				if not string.find(DynCore.Shared.Items[tostring(v["icon"])].image, "//") and not string.find(v["icon"], "//") then
                    v["icon"] = "nui://dyn-inventory/html/images/"..DynCore.Shared.Items[tostring(v["icon"])].image
				end
			end
		end
	end
...
RegisterNetEvent('dyn-menu:client:openMenu', function(data, sort, skipFirst)
    openMenu(data, sort, skipFirst)
end)

RegisterNetEvent('dyn-menu:client:closeMenu', function()
    closeMenu()
end)
        ...
        if data then
            if data.params.event then
                ...
                elseif data.params.isQBCommand then
                    TriggerServerEvent('DynCore:CallCommand', data.params.event, data.params.args)
...
RegisterNUICallback('closeMenu', function(_, cb)
    ...
    SetNuiFocus(false)
    cb('ok')
    TriggerEvent("dyn-menu:client:menuClosed")
end)
