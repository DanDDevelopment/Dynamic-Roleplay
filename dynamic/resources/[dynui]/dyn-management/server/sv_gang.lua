...
-- Get Employees
DynCore.Functions.CreateCallback('dyn-gangmenu:server:GetEmployees', function(source, cb, gangname)
	local src = source
...
RegisterNetEvent('dyn-gangmenu:server:stash', function()
	local src = source
	local Player = DynCore.Functions.GetPlayer(src)
...
-- Grade Change
RegisterNetEvent('dyn-gangmenu:server:GradeUpdate', function(data)
	local src = source
	local Player = DynCore.Functions.GetPlayer(src)
	...
	if Employee then
		if Employee.Functions.SetGang(Player.PlayerData.gang.name, data.grade) then
			TriggerClientEvent('DynCore:Notify', src, 'Successfully promoted!', 'success')
			...
			TriggerClientEvent('DynCore:Notify', src, 'Grade does not exist.', 'error')
		end
	end
	TriggerClientEvent('dyn-gangmenu:client:OpenMenu', src)
end)

-- Fire Member
RegisterNetEvent('dyn-gangmenu:server:FireMember', function(target)
	local src = source
	local Player = DynCore.Functions.GetPlayer(src)
	...
	if Employee then
		if target == Player.PlayerData.citizenid then
			TriggerClientEvent('DynCore:Notify', src, 'You can\'t kick yourself out of the gang!', 'error')
			return
		...
		if Employee.Functions.SetGang('none', '0') then
			Employee.Functions.Save()
			TriggerEvent('dyn-log:server:CreateLog', 'gangmenu', 'Gang Fire', 'orange', Player.PlayerData.charinfo.firstname .. ' ' .. Player.PlayerData.charinfo.lastname .. ' successfully fired ' .. Employee.PlayerData.charinfo.firstname .. ' ' .. Employee.PlayerData.charinfo.lastname .. ' (' .. Player.PlayerData.gang.name .. ')', false)
			TriggerClientEvent('DynCore:Notify', src, 'Gang Member fired!', 'success')

			if Employee.PlayerData.source then -- Player is online
				TriggerClientEvent('DynCore:Notify', Employee.PlayerData.source, 'You have been expelled from the gang!', 'error')
			end
		else
...
-- Recruit Player
RegisterNetEvent('dyn-gangmenu:server:HireMember', function(recruit)
	local src = source
	local Player = DynCore.Functions.GetPlayer(src)
	...
	if Target and Target.Functions.SetGang(Player.PlayerData.gang.name, 0) then
		TriggerClientEvent('DynCore:Notify', src, 'You hired ' .. (Target.PlayerData.charinfo.firstname .. ' ' .. Target.PlayerData.charinfo.lastname) .. ' come ' .. Player.PlayerData.gang.label .. '', 'success')
		TriggerClientEvent('DynCore:Notify', Target.PlayerData.source, 'You have been hired as ' .. Player.PlayerData.gang.label .. '', 'success')
		TriggerEvent('dyn-log:server:CreateLog', 'gangmenu', 'Recruit', 'yellow', (Player.PlayerData.charinfo.firstname .. ' ' .. Player.PlayerData.charinfo.lastname) .. ' successfully recruited ' .. Target.PlayerData.charinfo.firstname .. ' ' .. Target.PlayerData.charinfo.lastname .. ' (' .. Player.PlayerData.gang.name .. ')', false)
	end
	TriggerClientEvent('dyn-gangmenu:client:OpenMenu', src)
end)

-- Get closest player sv
DynCore.Functions.CreateCallback('dyn-gangmenu:getplayers', function(source, cb)
	local src = source
	local players = {}
	local PlayerPed = GetPlayerPed(src)
