...
local PlayerGang = DynCore.Functions.GetPlayerData().gang
...
AddEventHandler('onResourceStart', function(resource) --if you restart the resource
    if resource == GetCurrentResourceName() then
        Wait(200)
        PlayerGang = DynCore.Functions.GetPlayerData().gang
    end
end)
...
RegisterNetEvent('DynCore:Client:OnGangUpdate', function(InfoGang)
    PlayerGang = InfoGang
end)

RegisterNetEvent('dyn-gangmenu:client:Warbobe', function()
...
RegisterNetEvent('dyn-gangmenu:client:OpenMenu', function()
    shownGangMenu = true
    local gangMenu = {
        {
        ...
        },
        {
            ...
            params = {
                event = 'dyn-gangmenu:client:ManageGang',
            }
        },
        {
            ...
            params = {
                event = 'dyn-gangmenu:client:HireMembers',
            }
        },
        {
            ...
            params = {
                isServer = true,
                event = 'dyn-gangmenu:server:stash',
            }
        },
        {
            ...
            params = {
                event = 'dyn-gangmenu:client:Warbobe',
            }
        }
    }
    ...
    gangMenu[#gangMenu + 1] = {
        ...
        params = {
            event = 'dyn-menu:closeMenu',
        }
    }

    exports['dyn-menu']:openMenu(gangMenu)
end)

RegisterNetEvent('dyn-gangmenu:client:ManageGang', function()
    ...
    DynCore.Functions.TriggerCallback('dyn-gangmenu:server:GetEmployees', function(cb)
        for _, v in pairs(cb) do
            GangMembersMenu[#GangMembersMenu + 1] = {
                ...
                params = {
                    event = 'dyn-gangmenu:lient:ManageMember',
                    args = {
                        player = v,
                        work = PlayerGang
                    }
        ...
        GangMembersMenu[#GangMembersMenu + 1] = {
            ...
            icon = 'fa-solid fa-angle-left',
            params = {
                event = 'dyn-gangmenu:client:OpenMenu',
            }
        }
        exports['dyn-menu']:openMenu(GangMembersMenu)
    end, PlayerGang.name)
end)

RegisterNetEvent('dyn-gangmenu:lient:ManageMember', function(data)
    local MemberMenu = {
        {
    ...
    for k, v in pairs(DynCore.Shared.Gangs[data.work.name].grades) do
        MemberMenu[#MemberMenu + 1] = {
            header = v.name,
            txt = Lang:t('bodygang.grade') .. k,
            params = {
                isServer = true,
                event = 'dyn-gangmenu:server:GradeUpdate',
    ...
    MemberMenu[#MemberMenu + 1] = {
        ...
        params = {
            isServer = true,
            event = 'dyn-gangmenu:server:FireMember',
    ...
    exports['dyn-menu']:openMenu(MemberMenu)
end)

RegisterNetEvent('dyn-gangmenu:client:HireMembers', function()
    ...
    DynCore.Functions.TriggerCallback('dyn-gangmenu:getplayers', function(players)
        for _, v in pairs(players) do
            if v and v ~= PlayerId() then
                HireMembersMenu[#HireMembersMenu + 1] = {
                    ...
                    params = {
                        isServer = true,
                        event = 'dyn-gangmenu:server:HireMember',
                        args = v.sourceplayer
                    }
                }
            end
        ...
        exports['dyn-menu']:openMenu(HireMembersMenu)
    end)
end)
    ...
    if Config.UseTarget then
        for gang, zones in pairs(Config.GangMenus) do
            for index, coords in ipairs(zones) do
                ...
                exports['dyn-target']:AddCircleZone(zoneName, coords, 0.5, {
                    name = zoneName,
                    debugPoly = false,
                    useZ = true
                }, {
                            ...
                            if #(pos - coords) < 5.0 then
                                ...
                                if #(pos - coords) <= 1.5 then
                                    ...
                                    if not shownGangMenu then
                                        exports['dyn-core']:DrawText(Lang:t('drawtextgang.label'), 'left')
                                        shownGangMenu = true
                                    end

                                    if IsControlJustReleased(0, 38) then
                                        exports['dyn-core']:HideText()
                                        TriggerEvent('dyn-gangmenu:client:OpenMenu')
                                    end
                                end
