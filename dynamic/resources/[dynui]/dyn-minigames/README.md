...
## Quiz
```lua
  local success = exports['dyn-minigames']:Quiz({
...
## Word Guess
```lua
  local success = exports['dyn-minigames']:WordGuess('fivem', 'the game modification you are playing on', 5) -- how many wrong guesses allowed
  if success then print('success') else print('fail') end
```

## Word Scramble
```lua
  local success = exports['dyn-minigames']:WordScramble('fivem', 'the game modification you are playing on', 30) -- how long they have to unscramble in seconds
  if success then print('success') else print('fail') end
```

## Key Minigame
```lua
  local result = exports['dyn-minigames']:KeyMinigame(10) -- amount of presses they need to do
...
## Lockpick
```lua
  local success = exports['dyn-minigames']:Lockpick(5) -- number of tries
  if success then print('success') else print('fail') end
```

## Hacking
```lua
  local success = exports['dyn-minigames']:Hacking(5, 30) -- code block size & seconds to solve
  if success then print('success') else print('fail') end
```

## Skillbar
```lua
  local success = exports['dyn-minigames']:Skillbar() -- calling like this will use default easy difficulty with 1234
  if success then print('success') else print('fail') end

  local success = exports['dyn-minigames']:Skillbar('medium') -- calling like this will just change difficulty and still use 1234
  if success then print('success') else print('fail') end

  local success = exports['dyn-minigames']:Skillbar('easy', 'wasdfgh') -- calling like this will set difficulty and keys to press
  if success then print('success') else print('fail') end
```

## Pinpad
```lua
  local result = exports['dyn-minigames']:StartPinpad(1234) -- numbers available to use are 1-9
